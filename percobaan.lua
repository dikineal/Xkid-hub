-- @XKID SCRIPT V2.0 (Final Optimization + Updates)
-- by @WTF.XKID | Roblox Build For Mobile/PC | Ready for All Executors
-- Update: WindUI multi-fallback, Executor detection, FPS 120, Anti-AFK proper, Config fallback, Auto-expand sections
...
-- Coba method 1: Raw GitHub (tanpa bypass)
local urls = {
    ...
    "https://raw.githubusercontent.com/Footagesus/WindUI/master/main.lua"
}

for _, url in ipairs(urls) do
    if not WindUI then
        local success, result = pcall(function()
            print("[XKID] Trying: " .. url)
            local content = game:HttpGet(url) -- tanpa true parameter
        ...
        if success and result then
            ...
            if execSuccess and execResult then
                WindUI = execResult
                print("[XKID] WindUI loaded from: " .. url)
                break
            else
                loadError = execSuccess and "execution failed" or tostring(execResult)
            end
        else
...
TrackC(LP.CharacterAdded:Connect(function() task.wait(0.5); setOptimalFPS(120) end))

-- ================================ ANTI AFK PROPER (GERAK + KAMERA) ================================
...
local function sendProperAntiAFK()
    -- Method 1: VirtualInputManager (paling efektif)
    if VIM and VIM.SendKeyEvent then
        pcall(function()
            -- Kirim W (bergerak maju) sebentar
            VIM:SendKeyEvent(true, Enum.KeyCode.W, false, game)
            task.wait(0.1)
            VIM:SendKeyEvent(false, Enum.KeyCode.W, false, game)
            -- Kirim A (bergerak kiri) sebentar
            task.wait(0.05)
    ...
    -- Method 3: Gerakan kamera (fallback)
    pcall(function()
...
local function startAFK()
    ...
    notify("Anti AFK", "ON (Proper)", 1.5, "shield-check")
end
...
local function toggleSmartTP(v)
    ...
    if v then
        ...
        pcall(function()
            if Teleport.tool then
                Teleport.tool:Destroy()
                Teleport.tool = nil
            end
        end)
        Teleport.toolActive = false
    ...
    RunService:BindToRenderStep("XKIDSpec", Enum.RenderPriority.Camera.Value + 1, function()
        ...
        pcall(function()
            ...
            if Spec.mode == "third" then
                ...
                Camera.CFrame = CFrame.new(
                    (CFrame.new(targetHrp.Position) * CFrame.Angles(0, math.rad(-Spec.orbitYaw), 0) * CFrame.Angles(math.rad(-Spec.orbitPitch), 0, 0) * CFrame.new(0, 0, Spec.dist)).Position,
...
local function startAutoLike()
    ...
    State.AutoLike.thread = task.spawn(function()
        while State.AutoLike.active and getgenv()._XKID_RUNNING do
            ...
            if ok then
                notify("Auto Like", result .. " | Total: " .. State.AutoLike.count, 1.5, "heart")
            end
...
secTargetTP:Button({ Title = "Execute TP", Desc = "Teleport to target", Callback = function()
    pcall(function()
        if tpTarget == "" then
            notify("Teleport", "Input target!", 2, "circle-alert")
            return
        end
        local target = nil
        for _,p in pairs(Players:GetPlayers()) do
        ...
        if not target or not target.Parent or not target.Character then
            notify("Teleport", "Invalid Target", 2, "circle-alert")
            return
        end
...
for i = 1, 3 do
    ...
    hc:Button({ Title = "📍 Load " .. idx, Callback = function()
        if not SavedLocs[idx] then
            notify("Slot " .. idx, "Empty", 1.5, "save")
            return
        end
        local r = getRoot()
        if not r then return end
...
secESPCol:Dropdown({ Title = "Normal Color", Values = { "Hijau", "Merah", "Biru", "Kuning", "Ungu", "Cyan", "Orange", "Pink", "Putih", "Hitam" }, Default = "Hijau", Callback = function(v)
    if colorMap[v] then
        ...
        State.ESP.boxColor_N = colorMap[v]
    end
...
chatTargetDrop = secChat:Dropdown({ Title = "Select Targets", Multi = true, AllowNone = true, Values = getDisplayNames(), Callback = function(selected)
...
task.spawn(function()
    ...
    local function connectLegacyChat(player)
        pcall(function()
            TrackC(player.Chatted:Connect(function(msg) onChat(player.DisplayName, msg) end))
        end)
    end
...
secSrv:Button({ Title = "Server Hop", Desc = "Find a new server", Callback = function()
    pcall(function()
        ...
        if not req then
            notify("Error", "HTTP not supported", 2, "circle-alert")
            return
        end
        local res = req({
...
local configDrop = secFile:Dropdown({ Title = "Load Config", Values = getConfigList(), Callback = function(selected)
