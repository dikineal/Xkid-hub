...
========================
  ...
  📱 Tiktok: @wtf.xkid
  💬 Discord: @4Sharken
  📌 v1.0.4 — Anti-Banwave Edition (Fixed)
]]
    ...
    if not AntiAFK.heartbeatThread then
        AntiAFK.heartbeatThread = task.spawn(function()
            while State.Security.afkActive and getgenv()._XKID_RUNNING do
                ...
                local lastInput = tick() -- fallback jika GetLastInputTime tidak ada
...
local function initPlayerCache(player)
    ...
    local cache = { texts = nil, tracer = nil, boxLines = {}, hl = nil }
    pcall(function()
...
T_HOME:Section({ Title = "System Access", Opened = true, Box = true }):Paragraph({ Title = "Identity Data", Desc = "\"Talk is cheap. Show me the code. 💻\"\n\n[ 👤 ] Operator: @WTF.XKID\n[ 📱 ] TikTok: @wtf.xkid\n[ 💬 ] Discord: @4Sharken" })
T_HOME:Section({ Title = "Discord", Opened = true, Box = true }):Button({ Title = "Copy Discord Link", Desc = "discord.gg/bzumc2u96", Icon = "copy", Callback = function() pcall(function() setclipboard("https://discord.gg/bzumc2u96") end) end })
...
task.spawn(function()
    ...
    while getgenv()._XKID_RUNNING do
        task.wait(0.5)
        pcall(function() if srvLabel and cachedMapName then local pCount, mCount = #Players:GetPlayers(), Players.MaxPlayers; local uptime = formatTime(os.difftime(os.time(), START_TIME)); local job = game.JobId ~= "" and game.JobId:sub(1, 8) .. "..." or "N/A"; srvLabel:SetDesc(string.format("Grid: %s\nNode: %s\nEntities: %d / %d\nSession: %s", cachedMapName, job, pCount, mCount, uptime)) end end)
        ...
        pcall(function() if securityLabel then local afk = State.Security.afkActive and "🟢" or "🔴"; local sl = State.Security.shiftLock and "🟢" or "🔴"; local lag = State.Security.antiLag and "🟢" or "🔴"; securityLabel:SetDesc(string.format("AFK: %s | ShiftLock: %s | Boost: %s", afk, sl, lag)) end end)
    end
end)
...
T_AV:Section({ Title = "State Control", Opened = true, Box = true }):Button({ Title = "Fast Respawn 💀", Desc = "Respawn on death point", Icon = "skull", Callback = function() fastRespawn() end })
...
secTP:Button({ Title = "Execute TP ⚡", Desc = "Teleport to target", Icon = "navigation", Callback = function() pcall(function() if tpTarget == "" then notify("Teleport", "Input target!", 2); return end; local target = nil; for _, p in pairs(Players:GetPlayers()) do if p ~= LP and (string.find(string.lower(p.Name), string.lower(tpTarget)) or string.find(string.lower(p.DisplayName), string.lower(tpTarget))) then target = p; break end end; if not target or not target.Character then notify("Teleport", "Invalid Target", 2); return end; local tHrp = getCharRoot(target.Character); local myHrp = getRoot(); if not tHrp or not myHrp then return end; myHrp.CFrame = tHrp.CFrame * CFrame.new(0, 0, 3) + Vector3.new(0, 2, 0) end) end })
...
for i = 1, 3 do local idx = i
    secLoc:Button({ Title = "💾 Save Slot " .. idx, Icon = "save", Callback = function() local r = getRoot(); if not r then return end; SavedLocs[idx] = r.CFrame end })
    secLoc:Button({ Title = "📍 Load Slot " .. idx, Icon = "map-pin", Callback = function() if not SavedLocs[idx] then return end; local r = getRoot(); if not r then return end; r.CFrame = SavedLocs[idx] end })
end
...
T_CAM:Section({ Title = "Zoom Override", Opened = true, Box = true }):Toggle({ Title = "Max Zoom Out", Value = false, Icon = "zoom-out", Callback = function(v) pcall(function() LP.CameraMaxZoomDistance = v and 100000 or 400 end) end })
local secSP = T_CAM:Section({ Title = "Spectator Mode", Opened = true, Box = true })
local specDropdown = secSP:Dropdown({ Title = "Select Target", Values = getDisplayNames(), SearchBarEnabled = false, Callback = function(v) local p = findPlayerByDisplay(tostring(v)); if p then Spec.target = p; if p.Character and p.Character:FindFirstChild("HumanoidRootPart") then local _, ry, _ = p.Character.HumanoidRootPart.CFrame:ToEulerAnglesYXZ(); Spec.orbitYaw = math.deg(ry); Spec.orbitPitch = 20; Spec.fpYaw = math.deg(ry) end end end })
secSP:Button({ Title = "Refresh Target List", Icon = "refresh-cw", Callback = function() pcall(function() specDropdown:Refresh(getDisplayNames(), true) end) end })
secSP:Toggle({ Title = "Enable Spectate", Value = false, Icon = "eye", Callback = function(v) Spec.active = v; if v then if not Spec.target or not Spec.target.Character then Spec.active = false; return end; Spec.origFov = Cam.FieldOfView; startSpecCapture(); startSpecLoop() else stopSpecLoop(); stopSpecCapture(); Cam.CameraType = Enum.CameraType.Custom; Cam.FieldOfView = Spec.origFov end end })
secSP:Toggle({ Title = "First Person View", Value = false, Icon = "view", Callback = function(v) Spec.mode = v and "first" or "third" end })
...
T_FREE:Section({ Title = "Cinematic Mode", Opened = true, Box = true }):Toggle({ Title = "Hide All UI", Value = false, Icon = "monitor-off", Callback = function(v) if v then State.Cinema.hideUI = true; State.Cinema.cachedGuis = {}; for _, gui in pairs(LP.PlayerGui:GetChildren()) do if gui:IsA("ScreenGui") and gui.Enabled then table.insert(State.Cinema.cachedGuis, gui); gui.Enabled = false end end; pcall(function() StarterGui:SetCoreGuiEnabled(Enum.CoreGuiType.All, false) end) else State.Cinema.hideUI = false; for _, gui in pairs(State.Cinema.cachedGuis) do if gui and gui.Parent then gui.Enabled = true end end; State.Cinema.cachedGuis = {}; pcall(function() StarterGui:SetCoreGuiEnabled(Enum.CoreGuiType.All, true) end) end end })
T_FREE:Section({ Title = "Cinematic Mode", Opened = true, Box = true }):Toggle({ Title = "Hide Name Player", Value = false, Icon = "user-x", Callback = function(v) State.Cinema.hideNamePlayer = v; if v then for _, p in ipairs(Players:GetPlayers()) do if p == LP then continue end; if p.Character then local hum = p.Character:FindFirstChildOfClass("Humanoid"); if hum then hum.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.None end; for _, desc in ipairs(p.Character:GetDescendants()) do if desc:IsA("BillboardGui") and desc.Parent then desc.Enabled = false end end end; if p.PlayerGui then for _, v in ipairs(p.PlayerGui:GetDescendants()) do if v:IsA("BillboardGui") then v.Enabled = false end end end end; if State.Cinema.hideNameConn then State.Cinema.hideNameConn:Disconnect() end; State.Cinema.hideNameConn = TrackC(RS.Heartbeat:Connect(function() for _, p in ipairs(Players:GetPlayers()) do if p == LP then continue end; if p.Character then local hum = p.Character:FindFirstChildOfClass("Humanoid"); if hum and hum.DisplayDistanceType ~= Enum.HumanoidDisplayDistanceType.None then hum.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.None end; for _, desc in ipairs(p.Character:GetDescendants()) do if desc:IsA("BillboardGui") and desc.Enabled then desc.Enabled = false end end end; if p.PlayerGui then for _, v in ipairs(p.PlayerGui:GetDescendants()) do if v:IsA("BillboardGui") and v.Enabled then v.Enabled = false end end end end end)) else if State.Cinema.hideNameConn then State.Cinema.hideNameConn:Disconnect(); State.Cinema.hideNameConn = nil end; for _, p in ipairs(Players:GetPlayers()) do if p == LP then continue end; if p.Character then local hum = p.Character:FindFirstChildOfClass("Humanoid"); if hum then hum.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.Viewer end; for _, desc in ipairs(p.Character:GetDescendants()) do if desc:IsA("BillboardGui") and desc.Parent then desc.Enabled = true end end end; if p.PlayerGui then for _, v in ipairs(p.PlayerGui:GetDescendants()) do if v:IsA("BillboardGui") then v.Enabled = true end end end end end end })

-- ══════════════════════════════════════════════════════════════
--  TAB 6: FILTER
-- ══════════════════════════════════════════════════════════════
local T_WO = Window:Tab({ Title = "Filter", Icon = "layers", ShowTabTitle = true, Border = true })
T_WO:Section({ Title = "Presets", Opened = true, Box = true }):Dropdown({ Title = "Select Filter", Values = { "Default", "Mendung HD", "Cool Blue HD", "Edgy HD", "Full Bright HD", "Senja", "Night HD" }, Value = "Default", SearchBarEnabled = false, Callback = function(v) applyFilter(v) end })
local secAtmos = T_WO:Section({ Title = "Atmosphere", Opened = false, Box = true })
local bloomActive = false
secAtmos:Toggle({ Title = "Bloom ON/OFF", Value = false, Icon = "sparkles", Callback = function(v) bloomActive = v; if v then local bloom = nil; for _, e in pairs(Lighting:GetChildren()) do if e:IsA("BloomEffect") and e.Name == "_XKID_FILTER" then bloom = e; break end end; if not bloom then bloom = Instance.new("BloomEffect", Lighting); bloom.Name = "_XKID_FILTER" end; bloom.Intensity = 0.5 else for _, e in pairs(Lighting:GetChildren()) do if e:IsA("BloomEffect") then e:Destroy() end end end end })
...
secAtmos:Slider({ Title = "Brightness", Step = 0.1, Value = { Min = 0, Max = 10, Default = originalLighting.Brightness }, Callback = function(v) Lighting.Brightness = v end })
secAtmos:Slider({ Title = "ClockTime", Step = 0.1, Value = { Min = 0, Max = 24, Default = originalLighting.ClockTime }, Callback = function(v) Lighting.ClockTime = v end })
secAtmos:Button({ Title = "Reset Atmosphere", Icon = "rotate-ccw", Callback = function() Lighting.Brightness = originalLighting.Brightness; Lighting.ClockTime = originalLighting.ClockTime; for _, e in pairs(Lighting:GetChildren()) do if e:IsA("ColorCorrectionEffect") then e.Contrast = 0 end; if e:IsA("BloomEffect") then e:Destroy() end end; bloomActive = false end })
...
T_ESP:Section({ Title = "Color Config", Opened = false, Box = true }):Dropdown({ Title = "ESP Color", Values = { "Hijau", "Merah", "Biru", "Kuning", "Ungu", "Cyan", "Putih" }, Value = "Hijau", SearchBarEnabled = false, Callback = function(v) if colorMap[v] then State.ESP.tracerColor_N = colorMap[v]; State.ESP.nameColor = colorMap[v] end end })
...
secChat:Toggle({ Title = "Enable Logger", Value = false, Icon = "file-text", Callback = function(v) State.Utility.chatLog = v; if not v then pcall(function() chatLogPanel:SetDesc("Logger disabled") end) end end })
secChat:Toggle({ Title = "Silent Mode", Value = false, Icon = "volume-x", Callback = function(v) State.Utility.chatSilent = v end })
...
secAutoLike:Toggle({ Title = "Auto Like", Value = false, Icon = "heart", Callback = function(v) if v then startAutoLike() else stopAutoLike() end end })
...
print("✅ XKID v" .. CURRENT_VERSION .. " - Executable | Ultra-Safe AFK + Auto Like")