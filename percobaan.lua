...
local function stopAntiAFK()
    ...
    if AntiAFK.kickConn then
        AntiAFK.kickConn:Disconnect()
        AntiAFK.kickConn = nil
    end
    if AntiAFK.promptConn then
        AntiAFK.promptConn:Disconnect()
        AntiAFK.promptConn = nil
    end
    ...
    if v then
        State.Teleport.clickConn = TrackC(UIS.InputBegan:Connect(function(inp, gp)
            ...
            if inp.UserInputType == Enum.UserInputType.MouseButton1 and UIS:IsKeyDown(Enum.KeyCode.LeftControl) then
                local m = LP:GetMouse()
                if m.Hit then
                    getRoot().CFrame = CFrame.new(m.Hit.Position + Vector3.new(0, 3.5, 0))
...
local function startFreecamLoop()
    RS:BindToRenderStep("XKIDFreecam", Enum.RenderPriority.Camera.Value + 1, function(dt)
        ...
        if rollTarget == 0 and math.abs(FC.rollDeg) < 1 and math.abs(I_RollVel) < 1 then
        ...
        pcall(function()
            ...
            if Spec.mode == "third" then
                ...
                Cam.CFrame = CFrame.new(
                    (CFrame.new(hrp.Position) * CFrame.Angles(0, math.rad(-Spec.orbitYaw), 0) * CFrame.Angles(math.rad(-Spec.orbitPitch), 0, 0) * CFrame.new(0, 0, Spec.dist)).Position,
...
local function applyFilter(filter)
    ...
    if filter == "Default" then
        notify("Filter", "Reset to game default ✅", 2)
        return
    end
...
local Window = WindUI:CreateWindow({
    ...
    User = {
        ...
        Anonymous = false,
        Callback = function()
            notify("System", "XKID Identity Verified ✅", 3)
        end,
    },
    ...
    Watermark = {
        Enabled = true,
        Text = "XKID v" .. CURRENT_VERSION .. " | Boreal",
...
secStateCtrl:Button({
    Title = "Fast Respawn 💀", Desc = "Respawn on death point", Icon = "skull",
    Callback = function() fastRespawn() end,
})

local secMov = T_AV:Section({ Title = "Movement", Opened = true, Box = true })
secMov:Slider({
    Title = "Walk Speed", Desc = "Current: " .. tostring(State.Move.ws), Step = 1,
    Value = { Min = 16, Max = 500, Default = 16 }, IsTooltip = true,
    Callback = function(v) State.Move.ws = v; if getHum() then getHum().WalkSpeed = v end end,
})
secMov:Slider({
    Title = "Jump Power", Desc = "Current: " .. tostring(State.Move.jp), Step = 1,
    Value = { Min = 50, Max = 500, Default = 50 }, IsTooltip = true,
    Callback = function(v) State.Move.jp = v; local h = getHum(); if h then h.UseJumpPower = true; h.JumpPower = v end end,
})
secMov:Toggle({
    Title = "Infinite Jump", Value = false, Type = "Toggle", Icon = "arrow-big-up",
    Callback = function(v)
        if v then
...
secAbi:Toggle({
    Title = "Fly ✈️", Value = false, Type = "Toggle", Icon = "bird",
    Callback = function(v) toggleFly(v) end,
})
secAbi:Slider({
    Title = "Fly Speed", Desc = "Current: " .. tostring(State.Move.flyS), Step = 1,
    Value = { Min = 10, Max = 300, Default = 60 }, IsTooltip = true,
    Callback = function(v) State.Move.flyS = v end,
})
local noclipConn = nil
secAbi:Toggle({
    Title = "NoClip", Value = false, Type = "Toggle", Icon = "ghost",
    Callback = function(v)
        State.Move.ncp = v
        if v then
...
secDirTP:Toggle({
    Title = "Smart Touch/Click TP", Value = false, Type = "Toggle", Icon = "pointer",
    Callback = toggleSmartTP,
})

local secTP = T_TP:Section({ Title = "Target Teleport", Opened = true, Box = true })
local tpTarget = ""
secTP:Input({
    Title = "Search Player", Placeholder = "Type name...", Icon = "search",
    Callback = function(v) tpTarget = v end,
})
secTP:Button({
    Title = "Execute TP ⚡", Desc = "Teleport to target", Icon = "navigation",
    Callback = function()
        pcall(function()
...
secTP:Button({
    Title = "Refresh List", Icon = "refresh-cw",
    Callback = function()
        pcall(function() tpDropdown:Refresh(getDisplayNames(), true) end)
...
for i = 1, 3 do
    local idx = i
    secLoc:Button({
        Title = "💾 Save Slot " .. idx, Desc = "Save current position", Icon = "save",
        Callback = function()
            local r = getRoot()
            if not r then notify("Error", "Character not found ⚠️", 2); return end
            SavedLocs[idx] = r.CFrame
            notify("Slot", "Slot " .. idx .. " saved ✅", 2)
        end,
    })
    secLoc:Button({
        Title = "📍 Load Slot " .. idx, Desc = "Teleport to saved position", Icon = "map-pin",
        Callback = function()
            if not SavedLocs[idx] then notify("Error", "Slot is empty ⚠️", 2); return end
            local r = getRoot()
            if not r then return end
            r.CFrame = SavedLocs[idx]
            notify("Slot", "Loaded slot " .. idx .. " ✅", 2)
        end,
    })
end
...
secZoom:Toggle({
    Title = "Max Zoom Out", Value = false, Type = "Toggle", Icon = "zoom-out",
    Callback = function(v)
...
secSP:Button({
    Title = "Refresh Target List", Icon = "refresh-cw",
    Callback = function()
        pcall(function() specDropdown:Refresh(getDisplayNames(), true) end)
        notify("System", "List updated ✅", 2)
    end,
})
secSP:Toggle({
    Title = "Enable Spectate", Value = false, Type = "Toggle", Icon = "eye",
    Callback = function(v)
        Spec.active = v
        if v then
            if not Spec.target or not Spec.target.Parent or not Spec.target.Character or not Spec.target.Character:FindFirstChild("HumanoidRootPart") then
...
secSP:Toggle({
    Title = "First Person View", Value = false, Type = "Checkbox", Icon = "view",
    Callback = function(v) Spec.mode = v and "first" or "third" end,
})
secSP:Slider({
    Title = "Distance", Desc = "Current: 8", Step = 1,
    Value = { Min = 3, Max = 30, Default = 8 }, IsTooltip = true,
...
secFC:Toggle({
    Title = "Enable Freecam", Value = false, Type = "Toggle", Icon = "camera",
    Callback = function(v)
        FC.active = v
        if v then
            local cf = Cam.CFrame
            ...
            if hrp then
                ...
                FC.lockGyro.MaxTorque = Vector3.new(9e9, 9e9, 9e9)
                FC.lockGyro.CFrame = hrp.CFrame
                FC.lockPos = Instance.new("BodyPosition", hrp)
                FC.lockPos.MaxForce = Vector3.new(9e9, 9e9, 9e9)
                FC.lockPos.Position = hrp.Position
                hrp.Anchored = true
            end
...
secFC:Slider({
    Title = "Camera Speed", Desc = "Current: 3", Step = 0.5,
    Value = { Min = 1, Max = 20, Default = 3 }, IsTooltip = true,
    Callback = function(v) FC.speed = v end,
})
secFC:Slider({
    Title = "Sensitivity", Desc = "Current: 0.25", Step = 0.05,
    Value = { Min = 0.1, Max = 1.0, Default = 0.25 }, IsTooltip = true,
    Callback = function(v) FC.sens = v end,
})
...
secCine:Toggle({
    Title = "Hide All UI", Value = false, Type = "Toggle", Icon = "monitor-off",
    Callback = function(v)
        if v then
...
secCine:Toggle({
    Title = "Hide Name Player", Value = false, Type = "Toggle", Icon = "user-x",
    Callback = function(v)
        ...
        if v then
            ...
            if State.Cinema.hideNameConn then
                State.Cinema.hideNameConn:Disconnect()
                State.Cinema.hideNameConn = nil
            end
            for _, p in ipairs(Players:GetPlayers()) do
                if p == LP then continue end
                if p.Character then
...
secAtmos:Toggle({
    Title = "Bloom ON/OFF", Value = false, Type = "Toggle", Icon = "sparkles",
    Callback = function(v)
        bloomActive = v
        if v then
            local bloom = nil
            for _, e in pairs(Lighting:GetChildren()) do
                if e:IsA("BloomEffect") and e.Name == "_XKID_FILTER" then bloom = e; break end
            end
...
secAtmos:Slider({
    Title = "Bloom Intensity", Step = 0.1, Value = { Min = 0, Max = 5, Default = 0.5 }, IsTooltip = true,
    Callback = function(v)
        if not bloomActive then return end
        for _, e in pairs(Lighting:GetChildren()) do
            if e:IsA("BloomEffect") then e.Intensity = v; break end
        end
    end,
})
secAtmos:Slider({
    Title = "Brightness", Step = 0.1, Value = { Min = 0, Max = 10, Default = originalLighting.Brightness }, IsTooltip = true,
    Callback = function(v) Lighting.Brightness = v end,
})
secAtmos:Slider({
    Title = "Exposure", Step = 0.1, Value = { Min = -5, Max = 5, Default = originalLighting.ExposureCompensation }, IsTooltip = true,
    Callback = function(v) Lighting.ExposureCompensation = v end,
})
secAtmos:Slider({
    Title = "ClockTime", Step = 0.1, Value = { Min = 0, Max = 24, Default = originalLighting.ClockTime }, IsTooltip = true,
    Callback = function(v) Lighting.ClockTime = v end,
})
secAtmos:Slider({
    Title = "Contrast", Step = 0.1, Value = { Min = -2, Max = 2, Default = 0 }, IsTooltip = true,
    Callback = function(v)
        for _, e in pairs(Lighting:GetChildren()) do
            if e:IsA("ColorCorrectionEffect") then e.Contrast = v; return end
        end
        local cc = Instance.new("ColorCorrectionEffect", Lighting)
        cc.Name = "_XKID_FILTER"
        cc.Contrast = v
...
secAtmos:Button({
    Title = "Reset Atmosphere", Icon = "rotate-ccw",
    Callback = function()
        Lighting.Brightness = originalLighting.Brightness
...
secGfx:Slider({
    Title = "Quality Level", Step = 1, Value = { Min = 1, Max = 10, Default = 1 }, IsTooltip = true,
    Callback = function(v)
        if gfxMap[v] then pcall(function() settings().Rendering.QualityLevel = Enum.QualityLevel[gfxMap[v]] end) end
    end,
})
secGfx:Dropdown({
    Title = "FPS Cap", Values = { "30", "60", "120", "144", "240", "Unlimited" }, Value = "60", SearchBarEnabled = false,
    Callback = function(v)
...
secESP:Toggle({
    Title = "Enable Radar", Value = false, Type = "Toggle", Icon = "radar",
    Callback = function(v)
        State.ESP.active = v
...
secESP:Dropdown({
    Title = "Tracer Origin", Values = { "Bottom", "Center", "Mouse", "OFF" }, Value = "Bottom", SearchBarEnabled = false,
    Callback = function(v) State.ESP.tracerMode = v end,
})
secESP:Toggle({
    Title = "Highlight Entity", Value = false, Type = "Checkbox", Icon = "highlighter",
    Callback = function(v) State.ESP.highlightMode = v end,
})
secESP:Slider({
    Title = "Scan Distance", Desc = "Current: 300", Step = 10,
    Value = { Min = 50, Max = 500, Default = 300 }, IsTooltip = true,
...
secESPColor:Dropdown({
    Title = "Normal Color", Values = { "Hijau", "Merah", "Biru", "Kuning", "Ungu", "Cyan", "Orange", "Pink", "Putih", "Hitam" },
    Value = "Hijau", SearchBarEnabled = false,
    Callback = function(v) if colorMap[v] then State.ESP.tracerColor_N = colorMap[v] end end,
})
secESPColor:Dropdown({
    Title = "Suspect Color", Values = { "Merah", "Hijau", "Biru", "Kuning", "Ungu", "Cyan", "Orange", "Pink", "Putih", "Hitam", "Crimson" },
    Value = "Crimson", SearchBarEnabled = false,
    Callback = function(v) if colorMap[v] then State.ESP.tracerColor_S = colorMap[v]; State.ESP.boxColor_S = colorMap[v] end end,
})
secESPColor:Dropdown({
    Title = "Glitch Acc Color", Values = { "Orange", "Merah", "Hijau", "Biru", "Kuning", "Ungu", "Cyan", "Pink", "Putih", "Hitam" },
    Value = "Orange", SearchBarEnabled = false,
...
secChat:Toggle({
    Title = "Enable Logger", Value = false, Type = "Toggle", Icon = "file-text",
    Callback = function(v)
...
secChat:Toggle({
    Title = "Silent Mode", Value = false, Type = "Checkbox", Icon = "volume-x",
    Callback = function(v) State.Utility.chatSilent = v end,
})
...
secSrv:Toggle({
    Title = "Auto Rejoin 🔄", Value = false, Type = "Toggle", Icon = "refresh-cw",
    Callback = function(v)
        if v then
...
secSrv:Button({
    Title = "Force Rejoin", Desc = "Rejoin current server", Icon = "log-in",
    Callback = function()
        notify("System", "Rejoining...", 2)
        pcall(function() TPService:TeleportToPlaceInstance(game.PlaceId, game.JobId, LP) end)
    end,
...
secSrv:Button({
    Title = "Server Hop", Desc = "Find a new server", Icon = "shuffle",
    Callback = function()
...
secPerf:Toggle({
    Title = "FPS Boost ⚡", Value = false, Type = "Toggle", Icon = "zap",
    Callback = function(v)
...
secCamLock:Toggle({
    Title = "Force Shift Lock", Value = false, Type = "Toggle", Icon = "lock",
...
secCfg:Input({
    Title = "Config Name", Placeholder = "XKID_Config", Value = "XKID_Config",
...
local configDrop = secCfg:Dropdown({
    Title = "📂 Load Config", Values = getConfigList(), SearchBarEnabled = false,
    Callback = function(selected)
...
secCfg:Button({
    Title = "🗑️ Delete Config", Desc = "Delete selected config file", Icon = "trash-2",
    Callback = function()
        if currentConfig ~= "No config" and currentConfig ~= "" then
            pcall(function()
                if isfile and delfile and isfile("XKID_HUB/" .. currentConfig .. ".json") then
                    ...
                    notify("Config", currentConfig .. " deleted 🗑️", 2)
                    pcall(function() configDrop:Refresh(getConfigList(), true) end)
                    currentConfig = "No config"
                end
            end)
        else
            notify("Config", "Select config from Load List first! ⚠️", 2)
        end
    end,
})
secCfg:Button({
    Title = "🔄 Refresh Files", Desc = "Refresh config list", Icon = "refresh-cw",
    Callback = function()
        pcall(function() configDrop:Refresh(getConfigList(), true) end)
