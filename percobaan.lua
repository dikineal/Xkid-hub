...
local function notify(title, content, duration, icon)
    local ok = pcall(function() WindUI:Notify({ Title = title, Content = content, Duration = duration or 2, Icon = icon or "bell" }) end)
...
local function findPlayerByDisplay(s)
    if s=="[Self]" then return LP end
    for _,p in pairs(Players:GetPlayers()) do if p.DisplayName==s or p.Name==s then return p end end
    return nil
end
local function getCharRoot(c)
    if not c then return nil end
    return c:FindFirstChild("HumanoidRootPart") or c.PrimaryPart or c:FindFirstChild("Head") or c:FindFirstChild("Torso") or c:FindFirstChild("UpperTorso") or c:FindFirstChildWhichIsA("BasePart")
end
...
local function isOnGround()
    ...
    return workspace:Raycast(r.Position,Vector3.new(0,-5,0),p)~=nil
end
...
local function sendSilentInput()
    if VIM and VIM.SendKeyEvent then pcall(function() VIM:SendKeyEvent(true,Enum.KeyCode.F24,false,game) task.wait(0.05) VIM:SendKeyEvent(false,Enum.KeyCode.F24,false,game) end) return end
    if VirtualUser and VirtualUser.ClickButton2 then pcall(function() local vp=Camera.ViewportSize local center=vp and Vector2.new(vp.X/2,vp.Y/2) or Vector2.new(0,0) VirtualUser:ClickButton2(center) end) return end
    pcall(function() local cf=Camera.CFrame Camera.CFrame=cf*CFrame.Angles(0,math.rad(2),0) task.wait(0.05) Camera.CFrame=cf end)
end
local function startAFK()
    if AFKSystem.active then return end
    AFKSystem.active=true; State.Security.afkActive=true
    AFKSystem.thread=task.spawn(function() while AFKSystem.active do sendSilentInput() for i=1,55 do if not AFKSystem.active then break end task.wait(1) end end end)
...
task.spawn(function() task.wait(0.5) startAFK() end)

--[ SHIFT LOCK ]--
TrackC(LP.CharacterAdded:Connect(function(char) task.wait(0.5) local h=char:FindFirstChildOfClass("Humanoid") if h then if State.Move.ws~=16 then h.WalkSpeed=State.Move.ws end if State.Move.jp~=50 then h.UseJumpPower=true h.JumpPower=State.Move.jp end end if State.Security.shiftLock then task.wait(0.2) local hrp=getRoot() if hrp then if State.Security.shiftLockGyro then State.Security.shiftLockGyro:Destroy() end State.Security.shiftLockGyro=Instance.new("BodyGyro",hrp) State.Security.shiftLockGyro.MaxTorque=Vector3.new(9e9,9e9,9e9) State.Security.shiftLockGyro.P=50000 State.Security.shiftLockGyro.D=1000 end end end))
    ...
    if v then
        ...
        RunService:BindToRenderStep("XKIDShiftLock",Enum.RenderPriority.Camera.Value+2,function()
            ...
            if hrp2 and gyro and gyro.Parent==hrp2 then
                ...
                if flat.Magnitude>0.01 then gyro.CFrame=CFrame.new(hrp2.Position,hrp2.Position+flat) end
            end
        end)
...
local function toggleSmartTP(v)
    ...
    if v then
        pcall(function()
            local tool=Instance.new("Tool"); tool.Name="TP Tool"; tool.RequiresHandle=false; tool.Parent=LP.Backpack
            Teleport.tool=tool; Teleport.toolActive=false
        ...
        pcall(function() if Teleport.tool then Teleport.tool:Destroy() end end)
...
local function startAutoWalk()
    ...
    local h=getHum(); if h then h.WalkSpeed=State.Move.autoWalkSpeed end
    RunService:BindToRenderStep("XKIDAutoWalk",Enum.RenderPriority.Character.Value+1,function()
        ...
        local hrp=getRoot(); local h=getHum()
        ...
        hrp.CFrame=hrp.CFrame+dir*(State.Move.autoWalkSpeed/60)
    end)
...
local function stopAutoWalk()
    ...
    local h=getHum(); if h then h.WalkSpeed=State.Move.ws end
    notify("Auto Walk","OFF",1.5,"play")
end

--[ ESP ENGINE ]--
local function initPlayerCache(p)
    if State.ESP.cache[p] then return end
    local c={ texts=nil, tracer=nil, boxLines={}, hl=nil, isSuspect=false, isGlitch=false, reason="" }
    pcall(function()
        c.texts=Drawing.new("Text")
        if c.texts then c.texts.Center=true; c.texts.Outline=true; c.texts.Font=2; c.texts.Size=13; c.texts.ZIndex=2 end
        c.tracer=Drawing.new("Line")
        if c.tracer then c.tracer.Thickness=1.5; c.tracer.ZIndex=1 end
        for i=1,4 do local l=Drawing.new("Line"); if l then l.Thickness=1.5; l.ZIndex=1; c.boxLines[i]=l end end
    end)
    State.ESP.cache[p]=c
end
local function clearPlayerCache(p)
    local c=State.ESP.cache[p]; if not c then return end
    ...
    for _,data in ipairs(espsortedPlayers) do
        ...
        pcall(function()
            ...
            if State.ESP.tracerMode~="OFF" and c.tracer then
                ...
                elseif State.ESP.tracerMode=="Mouse" then local m=UserInputService:GetMouseLocation(); origin=Vector2.new(m.X,m.Y) end
        ...
        if useHl and hlCount<30 then
            hlCount=hlCount+1
            pcall(function()
                local top,topOn=Camera:WorldToViewportPoint(hrp.Position+Vector3.new(0,3,0)); local bot,botOn=Camera:WorldToViewportPoint(hrp.Position-Vector3.new(0,3.5,0))
...
local function toggleFly(v)
    if not v then
        ...
        local h=getHum(); if h then h.PlatformStand=false; h:ChangeState(Enum.HumanoidStateType.GettingUp); h.WalkSpeed=State.Move.ws; h.UseJumpPower=true; h.JumpPower=State.Move.jp end
        notify("Fly","OFF",1.5,"bird"); return
    end
    local hrp,h=getRoot(),getHum(); if not hrp or not h then return end
    State.Fly.active=true; h.PlatformStand=true; flyVel=Vector3.zero
    ...
    RunService:BindToRenderStep("XKIDFly",Enum.RenderPriority.Camera.Value+1,function()
        ...
        local cf=Camera.CFrame; local spd=State.Move.flyS; local move=Vector3.zero; local keys=State.Fly._keys or {}
        if onMobile then move=cf.LookVector*(-flyJoy.Y)+cf.RightVector*flyJoy.X
        else
        ...
        if State.Fly.bg and State.Fly.bg.Parent then State.Fly.bg.CFrame=CFrame.new(r.Position,r.Position+cf.LookVector) end
    end)
end
...
local function makeFCBtn(name,txt,pos,actionKey)
    local b=Instance.new("TextButton",FCUI); b.Name=name; b.Size=UDim2.new(0,44,0,44); b.Position=pos; b.BackgroundColor3=Color3.fromRGB(15,15,15); b.BackgroundTransparency=0.4; b.Text=txt; b.TextColor3=Color3.fromRGB(255,255,255); b.TextSize=18; b.Font=Enum.Font.GothamBold; b.AutoButtonColor=false; Instance.new("UICorner",b).CornerRadius=UDim.new(0,10)
    local uis=Instance.new("UIStroke",b); uis.Color=Color3.fromRGB(220,20,60); uis.Thickness=2; uis.Transparency=0.3
    local ind=Instance.new("Frame",b); ind.Name="Indicator"; ind.Size=UDim2.new(0,6,0,6); ind.Position=UDim2.new(0,4,0,4); ind.BackgroundColor3=Color3.fromRGB(60,60,60); Instance.new("UICorner",ind).CornerRadius=UDim.new(1,0)
...
local eyeBtn=Instance.new("TextButton",FCUI); eyeBtn.Name="BtnEye"; eyeBtn.Size=UDim2.new(0,44,0,44); eyeBtn.Position=UDim2.new(1,-107,0.5,-66); eyeBtn.BackgroundColor3=Color3.fromRGB(15,15,15); eyeBtn.BackgroundTransparency=0.6; eyeBtn.Text="👁"; eyeBtn.TextColor3=Color3.fromRGB(255,255,255); eyeBtn.TextSize=18; eyeBtn.Font=Enum.Font.GothamBold; eyeBtn.AutoButtonColor=false; Instance.new("UICorner",eyeBtn).CornerRadius=UDim.new(0,10); Instance.new("UIStroke",eyeBtn).Color=Color3.fromRGB(220,20,60); eyeBtn.MouseButton1Click:Connect(function() FC_UI_Hidden=not FC_UI_Hidden; eyeBtn.Text=FC_UI_Hidden and "👁‍🗨" or "👁"; for _,b in ipairs(fcButtons) do b.Visible=not FC_UI_Hidden end end)
local function startFreecamCapture()
    fcKeysHeld={}
    table.insert(fcConns, UserInputService.InputBegan:Connect(function(inp,gp) if gp then return end; fcKeysHeld[inp.KeyCode]=true; if inp.UserInputType==Enum.UserInputType.MouseButton2 then FC._mouseRot=true; UserInputService.MouseBehavior=Enum.MouseBehavior.LockCurrentPosition end end))
    table.insert(fcConns, UserInputService.InputEnded:Connect(function(inp) fcKeysHeld[inp.KeyCode]=false; if inp.UserInputType==Enum.UserInputType.MouseButton2 then FC._mouseRot=false; UserInputService.MouseBehavior=Enum.MouseBehavior.Default end end))
    table.insert(fcConns, UserInputService.InputChanged:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseMovement and FC._mouseRot then I_YawVel=I_YawVel-inp.Delta.X*FC.sens*120; I_PitchVel=I_PitchVel-inp.Delta.Y*FC.sens*120 end end))
...
local function stopFreecamCapture() for _,c in ipairs(fcConns) do c:Disconnect() end; fcConns={}; fcKeysHeld={}; FC._mouseRot=false; UserInputService.MouseBehavior=Enum.MouseBehavior.Default end
...
local function fullCleanupFreecam()
    ...
    local h=getHum(); local hrp=getRoot()
    if h then h.WalkSpeed=FC.savedWalkSpeed; h.UseJumpPower=true; h.JumpPower=FC.savedJumpPower end
    ...
    RunService:BindToRenderStep("XKIDSelfSpec",Enum.RenderPriority.Camera.Value+1,function()
        ...
        pcall(function()
            ...
            if SS.mode=="First Person" then
                ...
                Camera.CFrame=CFrame.new(origin)*CFrame.Angles(0,math.rad(SS.fpYaw),0)*CFrame.Angles(math.rad(SS.fpPitch),0,0)
            else
                if #ssPinch==0 and pan.Magnitude<0.01 then
                    ...
                    elseif SS.mode=="Top Down" then SS.orbitPitch=-75; SS.orbitYaw=SS.orbitYaw+dt*8*SS.speed end
                end
                ...
                Camera.CFrame=CFrame.new((CFrame.new(targetHrp.Position+Vector3.new(0,h,0))*CFrame.Angles(0,math.rad(-SS.orbitYaw),0)*CFrame.Angles(math.rad(-SS.orbitPitch),0,0)*CFrame.new(0,0,SS.radius)).Position, targetHrp.Position+Vector3.new(0,h,0))
            end
        end)
    end)
end
local function stopSelfSpecLoop()
    RunService:UnbindFromRenderStep("XKIDSelfSpec")
    Camera.CameraType=Enum.CameraType.Custom; Camera.FieldOfView=SS.origFov
    SS.active=false; SS.orbitYaw=0; SS.orbitPitch=20; SS.fpYaw=0; SS.fpPitch=0; SS.radius=8; SS.height=3; ssPan=Vector2.zero
end
local function toggleSelfSpec(v)
    if v then
        ...
        if Teleport.clickConn then Teleport.clickConn:Disconnect(); Teleport.clickConn=nil end
        SS.active=true; SS.origFov=Camera.FieldOfView; SS.orbitYaw=0; SS.orbitPitch=20; SS.fpYaw=0; SS.fpPitch=0; SS.radius=SS.radius or 8; SS.height=SS.height or 3
...
local function startSpecCapture()
    table.insert(specConns, UserInputService.InputBegan:Connect(function(inp,gp) if gp or not Spec.active or inp.UserInputType~=Enum.UserInputType.Touch then return end; table.insert(specPinch,inp); specTM=#specPinch==1 and inp or nil end))
    table.insert(specConns, UserInputService.InputChanged:Connect(function(inp) if not Spec.active or inp.UserInputType~=Enum.UserInputType.Touch then return end; if #specPinch==1 and inp==specTM then specPan=specPan+Vector2.new(inp.Delta.X,inp.Delta.Y) elseif #specPinch>=2 then local d=(specPinch[1].Position-specPinch[2].Position).Magnitude; if specPinchD then local diff=d-specPinchD; Camera.FieldOfView=math.clamp(Camera.FieldOfView-diff*0.15,10,120); if Spec.mode=="third" then Spec.dist=math.clamp(Spec.dist-diff*0.03,3,30) end end; specPinchD=d end end))
...
local function startSpecLoop()
    RunService:BindToRenderStep("XKIDSpec",Enum.RenderPriority.Camera.Value+1,function()
        ...
        pcall(function()
            ...
            else if not Spec.target or not Spec.target.Character then Spec.active=false; stopSpecLoop(); stopSpecCapture(); Camera.CameraType=Enum.CameraType.Custom; Camera.FieldOfView=Spec.origFov; return end; targetChar=Spec.target.Character; targetHrp=targetChar:FindFirstChild("HumanoidRootPart") end
            if not targetHrp then if not Spec.isSelf then Spec.active=false; stopSpecLoop(); stopSpecCapture(); Camera.CameraType=Enum.CameraType.Custom; Camera.FieldOfView=Spec.origFov end; return end
            Camera.CameraType=Enum.CameraType.Scriptable
            ...
            if Spec.mode=="third" then
                ...
                Camera.CFrame=CFrame.new((CFrame.new(targetHrp.Position)*CFrame.Angles(0,math.rad(-Spec.orbitYaw),0)*CFrame.Angles(math.rad(-Spec.orbitPitch),0,0)*CFrame.new(0,0,Spec.dist)).Position, targetHrp.Position+Vector3.new(0,1,0))
            else
                local head=targetChar:FindFirstChild("Head"); local origin=head and head.Position or targetHrp.Position+Vector3.new(0,1.5,0)
                Spec.fpYaw=Spec.fpYaw-pan.X*sens; Spec.fpPitch=math.clamp(Spec.fpPitch-pan.Y*sens,-85,85)
                Camera.CFrame=CFrame.new(origin)*CFrame.Angles(0,math.rad(Spec.fpYaw),0)*CFrame.Angles(math.rad(Spec.fpPitch),0,0)
            end
        end)
    end)
end
...
--[ AUTO LIKE ]--
local function getLikeRemotes() local r=ReplicatedStorage:FindFirstChild("Remotes"); if not r then return nil,nil end; return r:FindFirstChild("GetLikeDataRemote"), r:FindFirstChild("LikePlayerEvent") end
    ...
    for _,p in pairs(Players:GetPlayers()) do
        if p~=LP then
            if State.AutoLike.radius>0 and myRoot then
                ...
                if theirRoot and (theirRoot.Position-myRoot.Position).Magnitude<=State.AutoLike.radius then table.insert(targets,p) end
...
local function applyCustomFilter()
    resetFilterOnly()
    Lighting.Brightness=originalLighting.Brightness; Lighting.Ambient=originalLighting.Ambient; Lighting.OutdoorAmbient=originalLighting.OutdoorAmbient; Lighting.GlobalShadows=originalLighting.GlobalShadows; Lighting.ExposureCompensation=State.CustomFilter.exposure
    ...
    local bloom=Instance.new("BloomEffect",Lighting); bloom.Name="_XKID_FILTER"; bloom.Intensity=State.CustomFilter.bloomIntensity; bloom.Size=State.CustomFilter.bloomSize
    Lighting.ClockTime=State.CustomFilter.clockTime
    resetDOFOnly()
    if State.CustomFilter.dofIntensity>0 then local dof=Instance.new("DepthOfFieldEffect",Lighting); dof.Name="_XKID_DOF"; dof.FarIntensity=State.CustomFilter.dofIntensity; dof.FocusDistance=State.CustomFilter.dofDistance end
end
local function applyFilter(filterName)
    resetFilterOnly(); resetDOFOnly()
    Lighting.ClockTime=originalLighting.ClockTime; Lighting.Brightness=originalLighting.Brightness; Lighting.Ambient=originalLighting.Ambient; Lighting.OutdoorAmbient=originalLighting.OutdoorAmbient; Lighting.GlobalShadows=originalLighting.GlobalShadows; Lighting.ExposureCompensation=originalLighting.ExposureCompensation
    if filterName=="Default" then
        State.CustomFilter.tintR=255; State.CustomFilter.tintG=255; State.CustomFilter.tintB=255; State.CustomFilter.saturation=0; State.CustomFilter.contrast=0; State.CustomFilter.brightness=0; State.CustomFilter.exposure=0; State.CustomFilter.bloomIntensity=0; State.CustomFilter.bloomSize=24; State.CustomFilter.clockTime=14; State.CustomFilter.dofIntensity=0; State.CustomFilter.dofDistance=50; notify("Visuals","Default",1.5,"palette"); return
    end
    ...
    if preset then
        ...
        local bloom=Instance.new("BloomEffect",Lighting); bloom.Name="_XKID_FILTER"; bloom.Intensity=preset.bloomI or 0; bloom.Size=preset.bloomS or 24
        if preset.dofI and preset.dofI>0 then local dof=Instance.new("DepthOfFieldEffect",Lighting); dof.Name="_XKID_DOF"; dof.FarIntensity=preset.dofI; dof.FocusDistance=preset.dofD or 50 end
        State.CustomFilter.tintR=preset.tint.R*255; State.CustomFilter.tintG=preset.tint.G*255; State.CustomFilter.tintB=preset.tint.B*255; State.CustomFilter.saturation=preset.sat or 0; State.CustomFilter.contrast=preset.con or 0; State.CustomFilter.brightness=preset.bri or 0; State.CustomFilter.exposure=preset.exp or 0; State.CustomFilter.bloomIntensity=preset.bloomI or 0; State.CustomFilter.bloomSize=preset.bloomS or 24; State.CustomFilter.clockTime=preset.time or 14; State.CustomFilter.dofIntensity=preset.dofI or 0; State.CustomFilter.dofDistance=preset.dofD or 50
...
--[ GRADIENT CRIMSON TO PINK ]--
pcall(function() WindUI:RemoveTheme("Crimson") end)
local XKID_PINK_THEME = {
    Name = "XKID_CrimsonPink",
    ...
    Accent = WindUI:Gradient({
        ["0"] = { Color = Color3.fromRGB(220,20,60), Transparency = 0 },
        ["33"] = { Color = Color3.fromRGB(255,50,100), Transparency = 0 },
        ["66"] = { Color = Color3.fromRGB(255,80,140), Transparency = 0 },
        ["100"] = { Color = Color3.fromRGB(255,150,200), Transparency = 0 },
    }, { Rotation = 45, Transparency = 0 }),
}
WindUI:AddTheme(XKID_PINK_THEME)
WindUI:SetTheme("XKID_CrimsonPink")
...
secAutoWalk:Slider({ Title = "Walk Speed", Step = 1, Value = { Min = 1, Max = 100, Default = 16 }, Callback = function(v) State.Move.autoWalkSpeed=v; if State.Move.autoWalk then local h=getHum(); if h then h.WalkSpeed=v end end end })
...
secSP:Toggle({ Title = "Enable Spectate", Default = false, Callback = function(v) if SS.active then toggleSelfSpec(false) end; Spec.active=v; if v then if not Spec.target or not Spec.target.Character then if Spec.isSelf and LP.Character then else Spec.active=false; notify("Error","No target",2,"circle-alert") return end end; Spec.origFov=Camera.FieldOfView; startSpecCapture(); startSpecLoop(); notify("Spectator","ON",2,"eye") else stopSpecLoop(); stopSpecCapture(); Camera.CameraType=Enum.CameraType.Custom; Camera.FieldOfView=Spec.origFov; notify("Spectator","OFF",1.5,"eye") end end })
...
secFC:Toggle({ Title = "Enable Freecam", Default = false, Callback = function(v) if v and SS.active then toggleSelfSpec(false) end; FC.active=v; if v then local cf=Camera.CFrame; FC.pos=cf.Position; FC.pitchDeg=0; FC.yawDeg=0; FC.rollDeg=0; I_CamVel=Vector3.zero; I_YawVel=0; I_PitchVel=0; I_RollVel=0; heightVelocity=0; fcJoy=Vector2.zero; local h=getHum(); local hrp=getRoot(); if h then FC.savedWalkSpeed=h.WalkSpeed; FC.savedJumpPower=h.JumpPower; h.WalkSpeed=0; h.JumpPower=0 end; if hrp then hrp.Anchored=true; FC.wasAnchored=true end; FC.origFov=Camera.FieldOfView; startFreecamCapture(); startFreecamLoop(); if getgenv()._XKID_FCUI then getgenv()._XKID_FCUI.Enabled=true end; FC_UI_Hidden=false; eyeBtn.Text="👁"; for _,b in ipairs(fcButtons) do b.Visible=true end; notify("Freecam","ON",2,"video") else fullCleanupFreecam(); notify("Freecam","OFF",1.5,"video") end end })
...
secProt:Button({ Title = "Stuck Fix", Desc = "Get unstuck from walls/ground", Callback = function() local hrp,h=getRoot(),getHum(); if hrp then hrp.Anchored=false; hrp.CFrame=hrp.CFrame+Vector3.new(0,3,0) end; if h then h.Sit=false; h:ChangeState(Enum.HumanoidStateType.Jumping) end; notify("Protection","Stuck fix applied",2,"wrench") end })
