...
local function formatTime(seconds) local hours = math.floor(seconds / 3600) local mins = math.floor((seconds % 3600) / 60) local secs = seconds % 60 if hours > 0 then return string.format("%02d:%02d:%02d", hours, mins, secs) else return string.format("%02d:%02d", mins, secs) end end
...
local function toggleFly(v)
    if not v then
        ...
        local hum = getHum() if hum then hum.PlatformStand = false hum:ChangeState(Enum.HumanoidStateType.GettingUp) hum.WalkSpeed = State.Move.ws hum.UseJumpPower = true hum.JumpPower = State.Move.jp hum.AutoRotate = true end
    ...
    RunService:BindToRenderStep("XKIDFly", Enum.RenderPriority.Camera.Value + 1, function()
        if not State.Fly.active then return end local r = getRoot() if not r then return end local camCF = Camera.CFrame local spd = State.Move.flyS local move = Vector3.zero local keys = State.Fly._keys or {}
        ...
        if State.Fly.bv and State.Fly.bv.Parent then State.Fly.bv.Velocity = flyVel end if State.Fly.bg and State.Fly.bg.Parent then State.Fly.bg.CFrame = CFrame.new(r.Position, r.Position + camCF.LookVector) end
    end)
end
...
local freecamLockBP = nil local freecamLockBG = nil local lockedPos = nil local lockedCF = nil local lockRenderStepConnected = false
    ...
    if lock then
        ...
        if not lockRenderStepConnected then
            RunService:BindToRenderStep("XKIDFreecamLock", Enum.RenderPriority.Last.Value, function()
                if not FC.active then return end local currentHrp = getRoot() if currentHrp and lockedPos then if freecamLockBP then freecamLockBP.Position = lockedPos end if freecamLockBG and lockedCF then freecamLockBG.CFrame = lockedCF end currentHrp.AssemblyLinearVelocity = Vector3.zero currentHrp.AssemblyAngularVelocity = Vector3.zero local currentPos = currentHrp.Position if math.abs(currentPos.Y - lockedPos.Y) > 0.05 then currentHrp.CFrame = CFrame.new(lockedPos.X, lockedPos.Y, lockedPos.Z) * CFrame.Angles(0, currentHrp.Orientation.Y, 0) end end
            end)
        ...
        local currentHrp = getRoot() local currentHum = getHum() if currentHum then currentHum.WalkSpeed = FC.savedWalkSpeed currentHum.JumpPower = FC.savedJumpPower end if currentHrp then currentHrp.AssemblyLinearVelocity = Vector3.zero currentHrp.AssemblyAngularVelocity = Vector3.zero end
...
local function toggleFreecam(v)
    if v then
        if State.Fly.active then toggleFly(false) end if State.Security.shiftLock then toggleShiftLock(false) end if State.Move.autoWalk then stopAutoWalk() end
        if State.SelfSpec.active then toggleSelfSpec(false) end if State.Spec.active then State.Spec.active = false stopSpecLoop() stopSpecCapture() end
        ...
        if getgenv()._XKID_FCUI then getgenv()._XKID_FCUI.Enabled = true end FC_UI_Hidden = false eyeBtn.Text = "👁" for _, b in ipairs(fcButtons) do b.Visible = true end
...
local function startSelfSpecLoop()
    ...
    RunService:BindToRenderStep("XKIDSelfSpec", Enum.RenderPriority.Camera.Value + 1, function()
        ...
        pcall(function()
            ...
            Camera.CameraType = Enum.CameraType.Scriptable local pan, sens = ssPan, onMobile and 0.2 or 0.3 ssPan = Vector2.zero
            if #ssPinch == 0 and pan.Magnitude < 0.01 then
                ...
                elseif SS.mode == "Orbit Steady" then SS.orbitYaw = SS.orbitYaw + dt * 25 * SS.speed SS.orbitPitch = 20
                elseif SS.mode == "Tilt Drift" then SS.orbitYaw = SS.orbitYaw + dt * 30 * SS.speed SS.orbitPitch = 20 + math.sin(tick() * SS.speed * 0.5) * 10 SS.roll = math.sin(tick() * SS.speed * 0.8) * 5
                end
            end
...
local function toggleSelfSpec(v)
    if v then
        if FC.active then toggleFreecam(false) end if State.Fly.active then toggleFly(false) end if State.Spec.active then State.Spec.active = false stopSpecLoop() stopSpecCapture() end
        if Teleport.clickConn then Teleport.clickConn:Disconnect() Teleport.clickConn = nil end
        SS.active = true SS.origFov = Camera.FieldOfView SS.orbitYaw = 0 SS.orbitPitch = 20 SS.radius = SS.radius or 8 SS.height = SS.height or 3
        startSSGesture() startSelfSpecLoop() notify("Self-Spectate", "ON — " .. (SS.mode or "Manual"), 2, "camera")
    else SS.active = false stopSSGesture() stopSelfSpecLoop() notify("Self-Spectate", "OFF", 1.5, "camera") end
end
...
local function startSpecLoop()
    RunService:BindToRenderStep("XKIDSpec", Enum.RenderPriority.Camera.Value + 1, function()
        ...
        pcall(function()
            ...
            else if not State.Spec.target or not State.Spec.target.Character then State.Spec.active = false stopSpecLoop() stopSpecCapture() Camera.CameraType = Enum.CameraType.Custom Camera.FieldOfView = State.Spec.origFov return end targetChar = State.Spec.target.Character targetHrp = targetChar:FindFirstChild("HumanoidRootPart") end
...
local function startHardFling()
    ...
    hardFlingConn = TrackC(RunService.Heartbeat:Connect(function()
        ...
        elseif State.HardFling.mode == "Shake" then if hardFlingBAV and hardFlingBAV.Parent then local shakeX = (math.random() - 0.5) * State.HardFling.currentPower * 0.5 local shakeY = (math.random() - 0.5) * State.HardFling.currentPower * 0.3 local shakeZ = (math.random() - 0.5) * State.HardFling.currentPower * 0.5 hardFlingBAV.AngularVelocity = Vector3.new(shakeX, shakeY, shakeZ) end end
