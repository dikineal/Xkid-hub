...
local function stopAntiAFK()
    ...
    if AntiAFK.heartbeatThread then
        task.cancel(AntiAFK.heartbeatThread)
        AntiAFK.heartbeatThread = nil
    end
...
TrackC(RS.RenderStepped:Connect(function()
    ...
    for _, data in ipairs(espsortedPlayers) do
        ...
        pcall(function()
            if c.texts then
                c.texts.Text = txt; c.texts.Color = cColor
            ...
            if State.ESP.tracerMode ~= "OFF" and c.tracer then
                ...
                elseif State.ESP.tracerMode == "Mouse" then
                    local m = UIS:GetMouseLocation(); origin = Vector2.new(m.X, m.Y)
                end
...
local function toggleFly(v)
    ...
    State.Fly.bg.MaxTorque = Vector3.new(9e9, 9e9, 9e9); State.Fly.bg.P = 50000
    startFlyCapture()
...
local FC = {
    ...
    speed = 3, sens = 0.25, savedCF = nil, origFov = 70, lockGyro = nil, lockPos = nil,
}
local I_CamVel = Vector3.zero; local I_YawVel = 0; local I_PitchVel = 0; local I_RollVel = 0; local heightVelocity = 0
local fcMoveTouch, fcMoveSt, fcJoy = nil, nil, Vector2.zero; local fcRotTouch, fcRotLast = nil, nil
...
FCUI.Name = "XKID_FreecamUI"; FCUI.ResetOnSpawn = false; FCUI.ZIndexBehavior = Enum.ZIndexBehavior.Global
FCUI.Enabled = false; FCUI.Parent = CoreGui; getgenv()._XKID_FCUI = FCUI
...
local function makeFCBtn(name, txt, pos, actionKey)
    ...
    local indicator = Instance.new("Frame", b); indicator.Name = "Indicator"; indicator.Size = UDim2.new(0, 8, 0, 8)
    indicator.Position = UDim2.new(0, 5, 0, 5); indicator.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
...
local function startFreecamCapture()
    ...
    table.insert(fcConns, UIS.TouchMoved:Connect(function(inp)
        if inp == fcRotTouch and fcRotLast then
            local dx, dy = inp.Position.X - fcRotLast.X, inp.Position.Y - fcRotLast.Y; fcRotLast = inp.Position
            I_YawVel = I_YawVel - dx * FC.sens * 80; I_PitchVel = I_PitchVel - dy * FC.sens * 80
        end
...
local function stopFreecamCapture()
    for _, c in ipairs(fcConns) do c:Disconnect() end; fcConns = {}
    fcMoveTouch = nil; fcMoveSt = nil; fcJoy = Vector2.zero; fcRotTouch = nil; fcRotLast = nil; fcKeysHeld = {}
...
local function startSpecCapture()
    ...
    table.insert(specConns, UIS.InputChanged:Connect(function(inp)
        ...
        elseif #specPinch >= 2 then
            ...
            if specPinchD then
                local diff = d - specPinchD; Cam.FieldOfView = math.clamp(Cam.FieldOfView - diff * 0.15, 10, 120)
...
local function stopSpecCapture()
    for _, c in ipairs(specConns) do c:Disconnect() end; specConns = {}
    specTM = nil; specPinch = {}; specPinchD = nil; specPan = Vector2.zero
end
    ...
    RS:BindToRenderStep("XKIDSpec", Enum.RenderPriority.Camera.Value + 1, function()
        ...
        pcall(function()
            ...
            if Spec.mode == "third" then
                ...
                local head = Spec.target.Character:FindFirstChild("Head"); local origin = head and head.Position or hrp.Position + Vector3.new(0, 1.5, 0)
...
local Window = WindUI:CreateWindow({
    ...
    User = {
        Enabled = true,
        Anonymous = false,
        Callback = function() notify("System", "XKID Identity Verified ✅", 3) end,
    },

    Topbar = { Height = 48, ButtonsType = "Default" },

    OpenButton = {
        Enabled = true, Title = "XKID", Icon = "terminal",
        Position = UDim2.new(0, 120, 0, 120), Draggable = true, OnlyMobile = false, Scale = 0.85,
    },

    Watermark = {
        Enabled = true, Text = "XKID v" .. CURRENT_VERSION .. " | Boreal",
        Position = "bottom-right", Opacity = 0.4, Size = 12,
    },
})
...
Window:SideBarButton({
    Title = "Rejoin", Icon = "refresh-cw", Variant = "Secondary",
    Callback = function()
...
Window:SideBarButton({
    Title = "Fast Respawn", Icon = "skull", Variant = "Secondary",
...
secDiscord:Button({
    Title = "Copy Discord Link", Desc = "discord.gg/bzumc2u96", Icon = "copy",
    Callback = function()
        pcall(function() setclipboard("https://discord.gg/bzumc2u96") end)
        notify("System", "Link disalin ✅", 2)
    end,
})
...
task.spawn(function()
    ...
    local function makeBarA(val, maxVal, len, mode)
        ...
        for i = 1, len do
            if i <= fill then local t = (i - 1) / math.max(1, len - 1)
    ...
    while getgenv()._XKID_RUNNING do
        ...
        pcall(function()
            if netLabel then
                ...
                local fpsBar = makeBarA(fps, 120, 14, "FPS"); local pingBar = makeBarA(ping, 200, 14, "PING")
...
secAbi:Toggle({ Title = "Soft Fling ⚡", Value = false, Type = "Toggle", Icon = "zap", Callback = function(v) State.SoftFling.active = v; State.Move.ncp = v; if v then if not softFlingConn then softFlingConn = TrackC(RS.Heartbeat:Connect(function() if not State.SoftFling.active then return end; local r = getRoot(); if not r then return end; pcall(function() r.AssemblyAngularVelocity = Vector3.new(0, State.SoftFling.power, 0); r.AssemblyLinearVelocity = Vector3.new(r.AssemblyLinearVelocity.X, 50, r.AssemblyLinearVelocity.Z) end); if LP.Character then for _, p in pairs(LP.Character:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide = false end end end end)) end else if softFlingConn then softFlingConn:Disconnect(); softFlingConn = nil end end end })
...
secMisc:Button({ Title = "Copy JobID", Desc = "Copy server Job ID", Icon = "copy", Callback = function() pcall(function() setclipboard(game.JobId) end); notify("Utility", "JobID copied ✅", 2) end })
