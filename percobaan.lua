-- @XKID V3.9 | Anti AFK fix + all features
...
if not executor.has_writefile then getgenv()._XKID_NO_SAVE = true warn("[XKID] writefile not supported") end
...
local DebugLog = {}
local function addLog(msg, level) if level ~= "ERROR" and level ~= "BUG" then return end local e = string.format("[%s] [%s] %s", os.date("%H:%M:%S"), level, msg) table.insert(DebugLog, e) if #DebugLog > 100 then table.remove(DebugLog, 1) end end
local function notify(title, content, duration, icon) pcall(function() WindUI:Notify({ Title = title, Content = content, Duration = duration or 2, Icon = icon or "bell" }) end) end
...
local function getConfigList() local list = {} if executor.has_isfolder and executor.has_listfiles then pcall(function() if isfolder and isfolder("XKID_HUB") then for _, file in ipairs(listfiles("XKID_HUB")) do if file:match("%.json$") then local name = file:match("([^/\\]+)%.json$") if name then table.insert(list, name) end end end end) end if #list == 0 then table.insert(list, "No config") end return list end
...
local function isUserActive() if UserInputService:IsKeyDown(Enum.KeyCode.W) or UserInputService:IsKeyDown(Enum.KeyCode.A) or UserInputService:IsKeyDown(Enum.KeyCode.S) or UserInputService:IsKeyDown(Enum.KeyCode.D) or UserInputService:IsKeyDown(Enum.KeyCode.Space) or UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) or UserInputService:IsKeyDown(Enum.KeyCode.MouseButton1) then return true end if onMobile then if #UserInputService:GetTouchPositions() > 0 then return true end end local hrp, hum = getRoot(), getHum() if hrp and hum and hum.MoveDirection.Magnitude > 0.1 then return true end return false end
...
local function stealthAntiAFK()
    -- priority 1: remote heartbeat
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    if remotes then
        for _, name in ipairs({"Heartbeat","Ping","ClientAlive","KeepAlive","Pong"}) do
    ...
    AFKSystem.thread = task.spawn(function()
        while AFKSystem.active do
            ...
            if isUserActive() then
                AFKSystem.lastActive = tick()
            elseif tick() - AFKSystem.lastActive >= 60 then
                stealthAntiAFK()
                AFKSystem.lastActive = tick()
            end
        end
    ...
    if v then
        ...
        RunService:BindToRenderStep("XKIDShiftLock", Enum.RenderPriority.Camera.Value + 2, function()
            ...
            if hrp2 and gyro and gyro.Parent == hrp2 then
                local fl = Vector3.new(Camera.CFrame.LookVector.X, 0, Camera.CFrame.LookVector.Z)
                if fl.Magnitude > 0.01 then gyro.CFrame = CFrame.new(hrp2.Position, hrp2.Position + fl) end
            end
        end)
            ...
            for _, p in pairs(Players:GetPlayers()) do
                if p ~= LP and p.Character then
                    ...
                    for _, v in pairs(p.Character:GetChildren()) do
                        ...
                        elseif v:IsA("Accessory") then local h = v:FindFirstChild("Handle") if h and h:IsA("BasePart") then if h.Size.Magnitude > 20 then isSus = true reason = "Huge Hat" break elseif h.Size.Magnitude > 10 or (h.Transparency < 0.1 and h.Material == Enum.Material.Neon) then isGlitch = true reason = "Glitch Acc" end end end
                    end
...
local function toggleFreecam(v)
    if v then
        ...
        notify("Freecam", "ON (Lock Position + Emote)", 2, "video")
    else
        FC.active = false
        fullCleanupFreecam()
...
task.spawn(function()
    ...
    notify("System", "XKID_HUB V3.9 AKTIF — Anti AFK Fixed", 3, "rocket")
