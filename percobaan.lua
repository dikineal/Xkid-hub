...
========================
  ...
  📱 Tiktok: @wtf.xkid
  💬 Discord: @4Sharken
  📌 v1.0.9 — Chat Logger Fixed + Manual AFK
]]
...
local function initPlayerCache(player)
    ...
    pcall(function()
        ...
        if cache.texts then cache.texts.Center = true; cache.texts.Outline = true; cache.texts.Font = 2; cache.texts.Size = 13 end
        cache.tracer = Drawing.new("Line")
        if cache.tracer then cache.tracer.Thickness = 1.5 end
        for i = 1, 4 do local line = Drawing.new("Line")
            if line then line.Thickness = 1.5; cache.boxLines[i] = line end
        end
    end)
...
T_WO:Section({ Title = "Presets", Opened = true, Box = true }):Dropdown({ Title = "Select Filter", Values = { "Default", "Mendung HD", "Cool Blue HD", "Soft Fade HD", "Edgy HD", "Full Bright HD", "Senja", "Night HD", "Golden Hour" }, Value = "Default", SearchBarEnabled = false, Callback = function(v) applyFilter(v) end })
...
secAtmos:Button({ Title = "Reset Atmosphere", Icon = "rotate-ccw", Callback = function() Lighting.Brightness = originalLighting.Brightness; Lighting.ClockTime = originalLighting.ClockTime; resetFilterOnly() end })
...
secESP:Toggle({ Title = "Enable Radar", Value = false, Icon = "radar", Callback = function(v) State.ESP.active = v end })
...
-- Chat Logger Event Listener (FIXED)
task.spawn(function()
    -- Tangkap SEMUA chat dulu, baru filter
...
secProt:Toggle({ Title = "Anti Kick", Value = true, Type = "Toggle", Icon = "shield-check", Callback = function(v)
    -- Anti-Kick selalu aktif, toggle ini hanya indikator
    notify("Security", v and "Anti-Kick active" or "Anti-Kick active (always on)", 2)
end})

secProt:Toggle({ Title = "Anti AFK", Value = false, Type = "Toggle", Icon = "clock", Callback = function(v)
    if v then startOffsideTimer() else stopOffsideTimer() end
end})
...
secPerf:Toggle({ Title = "FPS Boost", Value = false, Type = "Toggle", Icon = "zap", Callback = function(v) State.Security.antiLag = v; if v then pcall(function() settings().Rendering.QualityLevel = 1 end); Lighting.GlobalShadows = false; Lighting.Brightness = 1 else pcall(function() settings().Rendering.QualityLevel = 7 end); Lighting.GlobalShadows = true; Lighting.Brightness = originalLighting.Brightness end end })
...
print("✅ XKID v" .. CURRENT_VERSION .. " - Chat Logger Fixed | Manual AFK")