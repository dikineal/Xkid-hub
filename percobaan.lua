...
-- ══════════════════════════════════════════════════════════════
--  LOAD WINDUI (VERSI TERBARU - SESUAI TEMPLATE)
-- ══════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════
--  DROPDOWN STATE & REFRESH HELPER
-- ══════════════════════════════════════════════════════════════
local DropdownState = {}
local function RefreshDropdown(dropdown, key, getNewValues)
    local saved = DropdownState[key]
    local newValues = getNewValues()
    
    -- Update daftar nilai (akan mereset pilihan ke index 1)
    pcall(function() dropdown:SetValues(newValues) end)
    
    -- Kembalikan pilihan yang tersimpan jika masih valid
    if saved and saved ~= "" then
        local found = false
        for _, v in ipairs(newValues) do
            if v == saved then found = true; break end
        end
        if found then
            pcall(function() dropdown:SetValue(saved) end)
        end
    end
end
...
local function fastRespawn()
    ...
    task.spawn(function()
        ...
        conn = LP.CharacterAdded:Connect(function(newChar)
            ...
            if newHrp then newHrp.CFrame = savedCF + Vector3.new(0, 3.5, 0) end
...
local function refreshCharacter()
    ...
    task.spawn(function()
        ...
        task.delay(15, function() if not done then conn:Disconnect(); LP.RespawnLocation = prevRespawn; State.Avatar.isRefreshing = false end end)
    end)
end

-- ══════════════════════════════════════════════════════════════
--  ESP ENGINE (DIPERKECIL UNTUK MENGURANGI PANJANG - TETAP BERFUNGSI)
-- ══════════════════════════════════════════════════════════════
    ...
    for _, data in ipairs(espsortedPlayers) do
        ...
        pcall(function()
            ...
            if State.ESP.tracerMode ~= "OFF" and c.tracer then
                ...
                if State.ESP.tracerMode == "Center" then origin = center end
                if State.ESP.tracerMode == "Mouse" then local m = UIS:GetMouseLocation(); origin = Vector2.new(m.X, m.Y) end
                c.tracer.From = origin; c.tracer.To = Vector2.new(rootPos.X, rootPos.Y); c.tracer.Visible = true
            end
        end)
    end
end))
...
local function stopFlyCapture() for _, c in ipairs(flyConns) do c:Disconnect() end; flyConns = {}; flyMoveTouch = nil; flyMoveSt = nil; flyJoy = Vector2.zero end
local function toggleFly(v)
    if not v then
        ...
        local hum = getHum(); if hum then hum.PlatformStand = false; hum.WalkSpeed = State.Move.ws; hum.UseJumpPower = true; hum.JumpPower = State.Move.jp end
        return
    end
    ...
    RS:BindToRenderStep("XKIDFly", Enum.RenderPriority.Camera.Value + 1, function()
        ...
        if move.Magnitude > 0 then flyVel = flyVel:Lerp(move.Unit * spd, 0.15) else flyVel = flyVel:Lerp(Vector3.zero, 0.1) end
...
local function startFreecamLoop()
    RS:BindToRenderStep("XKIDFreecam", Enum.RenderPriority.Camera.Value + 1, function(dt)
        ...
        local hrp, hum = getRoot(), getHum(); if hrp and not hrp.Anchored then hrp.Anchored = true end; if hum then hum:ChangeState(Enum.HumanoidStateType.Physics) end
    end)
end
...
local function logMsg(displayName, msg)
    if not State.Utility.chatLog or not State.Utility.chatTarget then return end
...
-- ══════════════════════════════════════════════════════════════
--  MAIN WINDOW (TEMPLATE WINDUI TERBARU)
-- ══════════════════════════════════════════════════════════════
local Window = WindUI:CreateWindow({
    Title = "XKID", Subtitle = "Engine", Author = "by @WTF.XKID", Folder = "XKIDScript", Icon = "terminal", Theme = "Crimson",
    Size = UDim2.fromOffset(680, 460), MinSize = Vector2.new(560, 350), MaxSize = Vector2.new(850, 560),
    ToggleKey = Enum.KeyCode.RightShift, Resizable = true, AutoScale = true, NewElements = true,
    HideSearchBar = false, ScrollBarEnabled = false, SideBarWidth = 200,
...
secTP:Button({ Title = "Execute TP ⚡", Callback = function() pcall(function() if tpTarget == "" then notify("Teleport", "Input target!", 2); return end; local target = nil; for _, p in pairs(Players:GetPlayers()) do if p ~= LP and (string.find(string.lower(p.Name), string.lower(tpTarget)) or string.find(string.lower(p.DisplayName), string.lower(tpTarget))) then target = p; break end end; if not target or not target.Character then notify("Teleport", "Invalid Target", 2); return end; local tHrp = getCharRoot(target.Character); local myHrp = getRoot(); if not tHrp or not myHrp then notify("Teleport", "Failed", 2); return end; myHrp.CFrame = tHrp.CFrame * CFrame.new(0, 0, 3) + Vector3.new(0, 2, 0); notify("Teleport", "Teleporting to "..target.DisplayName.." ✅", 2) end) end})
...
secTP:Button({ Title = "Refresh List", Callback = function()
    RefreshDropdown(tpDropdown, "tpDropdown", getDisplayNames)
...
for i = 1, 3 do local idx = i
    secLoc:Button({ Title = "💾 Save Slot "..idx, Callback = function() local r = getRoot(); if not r then return end; SavedLocs[idx] = r.CFrame; notify("Slot", "Slot "..idx.." saved ✅", 2) end })
    secLoc:Button({ Title = "📍 Load Slot "..idx, Callback = function() if not SavedLocs[idx] then notify("Error", "Slot is empty", 2); return end; local r = getRoot(); if not r then return end; r.CFrame = SavedLocs[idx]; notify("Slot", "Loaded slot "..idx.." ✅", 2) end })
end
...
secZoom:Toggle({ Title = "Max Zoom Out", Value = false, Callback = function(v) pcall(function() LP.CameraMaxZoomDistance = v and 100000 or 400 end) end })
...
local function startSpecLoop()
    RS:BindToRenderStep("XKIDSpec", Enum.RenderPriority.Camera.Value + 1, function()
        ...
        pcall(function()
            if not Spec.target or not Spec.target.Parent or not Spec.target.Character or not Spec.target.Character:FindFirstChild("HumanoidRootPart") then Spec.active = false; stopSpecLoop(); stopSpecCapture(); Cam.CameraType = Enum.CameraType.Custom; Cam.FieldOfView = Spec.origFov; return end
            local hrp = Spec.target.Character.HumanoidRootPart; Cam.CameraType = Enum.CameraType.Scriptable; local pan = specPan; specPan = Vector2.zero
            if Spec.mode == "third" then Spec.orbitYaw = Spec.orbitYaw + pan.X * 0.3; Spec.orbitPitch = math.clamp(Spec.orbitPitch + pan.Y * 0.3, -75, 75); Cam.CFrame = CFrame.new((CFrame.new(hrp.Position) * CFrame.Angles(0, math.rad(-Spec.orbitYaw), 0) * CFrame.Angles(math.rad(-Spec.orbitPitch), 0, 0) * CFrame.new(0, 0, Spec.dist)).Position, hrp.Position + Vector3.new(0, 1, 0))
            else local head = Spec.target.Character:FindFirstChild("Head"); local origin = head and head.Position or hrp.Position + Vector3.new(0, 1.5, 0); Spec.fpYaw = Spec.fpYaw - pan.X * 0.3; Spec.fpPitch = math.clamp(Spec.fpPitch - pan.Y * 0.3, -85, 85); Cam.CFrame = CFrame.new(origin) * CFrame.Angles(0, math.rad(Spec.fpYaw), 0) * CFrame.Angles(math.rad(Spec.fpPitch), 0, 0) end
        end)
    end)
end
...
secSP:Button({ Title = "Refresh Target List", Callback = function()
    RefreshDropdown(specDropdown, "specDropdown", getDisplayNames)
    notify("System", "List updated ✅", 2)
end })
...
secFC:Toggle({ Title = "Enable Freecam", Value = false, Callback = function(v) FC.active = v; if v then local cf = Cam.CFrame; FC.pos = cf.Position; local rx, ry = cf:ToEulerAnglesYXZ(); FC.pitchDeg = math.deg(rx); FC.yawDeg = math.deg(ry); local hrp = getRoot(); if hrp then FC.savedCF = hrp.CFrame; pcall(function() if FC.lockGyro then FC.lockGyro:Destroy() end end); pcall(function() if FC.lockPos then FC.lockPos:Destroy() end end); FC.lockGyro = Instance.new("BodyGyro", hrp); FC.lockGyro.MaxTorque = Vector3.new(9e9, 9e9, 9e9); FC.lockGyro.CFrame = hrp.CFrame; FC.lockPos = Instance.new("BodyPosition", hrp); FC.lockPos.MaxForce = Vector3.new(9e9, 9e9, 9e9); FC.lockPos.Position = hrp.Position; hrp.Anchored = true end; FC.origFov = Cam.FieldOfView; startFreecamCapture(); startFreecamLoop(); if getgenv()._XKID_FCUI then getgenv()._XKID_FCUI.Enabled = true end else fullCleanupFreecam() end end})
...
secFilter:Dropdown({ Title = "Select Filter", Values = {"Default","Full Bright HD","Mendung HD","Cool Blue HD","Soft Fade HD","Edgy HD","Senja","Night HD","Golden Hour"}, Value = "Default", Callback = function(v) applyFilter(v) end })
...
secESP:Toggle({ Title = "Enable Radar", Value = false, Callback = function(v) State.ESP.active = v end })
...
secChat:Toggle({ Title = "Enable Logger", Value = false, Callback = function(v) State.Utility.chatLog = v end })
...
secChat:Button({ Title = "🔄 Refresh List", Callback = function()
    RefreshDropdown(chatTargetDrop, "chatTargetDrop", getDisplayNames)
end })
...
secProt:Button({ Title = "Stuck Fix 🔧", Callback = function() local hrp, hum = getRoot(), getHum(); if hrp then hrp.Anchored = false; hrp.CFrame = hrp.CFrame + Vector3.new(0, 3, 0) end; if hum then hum.Sit = false; hum:ChangeState(Enum.HumanoidStateType.Jumping) end end })

local secSrv = T_SEC:Section({ Title = "Server Control", Opened = true })
secSrv:Toggle({ Title = "Auto Rejoin 🔄", Value = false, Callback = function(v) if v then State.Security.arConn = TrackC(GuiService.ErrorMessageChanged:Connect(function(err) if err and err ~= "" then task.wait(1); pcall(function() TPService:TeleportToPlaceInstance(game.PlaceId, game.JobId, LP) end) end end)) else if State.Security.arConn then State.Security.arConn:Disconnect(); State.Security.arConn = nil end end end})
secSrv:Button({ Title = "Force Rejoin", Callback = function() pcall(function() TPService:TeleportToPlaceInstance(game.PlaceId, game.JobId, LP) end) end })
secSrv:Button({ Title = "Server Hop", Callback = function() pcall(function() local req = (syn and syn.request) or (http and http.request) or http_request or request; if not req then return end; local res = req({Url = "https://games.roblox.com/v1/games/"..game.PlaceId.."/servers/Public?sortOrder=Desc&limit=100", Method = "GET"}); if res.StatusCode == 200 then local body = HttpService:JSONDecode(res.Body); if body and body.data then for _, v in ipairs(body.data) do if v.playing > 0 and v.playing < v.maxPlayers and v.id ~= game.JobId then TPService:TeleportToPlaceInstance(game.PlaceId, v.id, LP); return end end end end end) end})
    ...
    pcall(function()
        if isfile and readfile and isfile("XKID_HUB/"..selected..".json") then
            ...
            if data then
                ...
                if data.ESP then State.ESP.tracerMode = data.ESP.tracerMode or "Bottom"; State.ESP.maxDrawDistance = data.ESP.maxDrawDistance or 300 end
...
secCfg:Button({ Title = "🗑️ Hapus Config", Callback = function()
    if currentConfig ~= "No config" and currentConfig ~= "" then
        pcall(function() if isfile and delfile and isfile("XKID_HUB/"..currentConfig..".json") then delfile("XKID_HUB/"..currentConfig..".json"); DropdownState["configDrop"] = "No config"; currentConfig = "No config"; RefreshDropdown(configDrop, "configDrop", getConfigList) end end)
    end
end})
secCfg:Button({ Title = "🔄 Refresh Files", Callback = function()
    RefreshDropdown(configDrop, "configDrop", getConfigList)
end })
...
print("✅ XKID Engine v"..CURRENT_VERSION.." - Dropdown Fixed: SetValues + SetValue fallback")