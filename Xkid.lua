--[[
  ╔═══════════════════════════════════════════════════════╗
  ║      🌾  I N D O   F A R M E R  v17.0  🌾           ║
  ║      XKID HUB  ✦  Aurora UI — Exact Syntax          ║
  ║      SavePos ✦  AutoHarvest ✦  Petir Shield ⚡       ║
  ╚═══════════════════════════════════════════════════════╝
  NPC CONFIRMED:
    ...
    for _, v in pairs(Workspace:GetDescendants()) do
        ...
        if isCrop and not visited[v] then
            ...
            if pp then
                ...
                elseif v:IsA("Model") and v.PrimaryPart then partPos = v.PrimaryPart.Position end
                if partPos then
                    local r = getRoot()
                    if r then r.CFrame = CFrame.new(partPos.X, partPos.Y+3, partPos.Z); task.wait(0.15) end
                    pcall(function() fireproximityprompt(pp) end)
                    harvested = harvested + 1; task.wait(0.2)
                end
            end
        end
    end
...
local function setupIntercepts()
    task.spawn(function()
        local r; for i=1,20 do r=getR("LightningStrike"); if r then break end; task.wait(1) end
        if not r then return end
        r.OnClientEvent:Connect(function()
            if not _G.PenangkalPetir then return end
            task.spawn(fleePetir)
        end)
        print("[ XKID ] ⚡ Petir intercept ready")
    end)
    task.spawn(function()
    ...
    task.spawn(function()
        ...
        r.OnClientEvent:Connect(function(msg)
            ...
            local ml = msg:lower()
            if ml:find("hujan") then notif("🌧 Hujan!", "Tanaman cepat tumbuh", 4)
            elseif ml:find("petir") or ml:find("gosong") then notif("⚡ Petir!", msg, 4)
        ...
        r.OnClientEvent:Connect(function(_, p41)
            if _G.AutoSell and type(p41)=="string" and p41:find("OPEN") then
                task.delay(0.6, function()
                    ...
                    notif(ok and "💰 Auto Sell ✅" or "❌ Sell", msg or "", 3)
                end)
            end
        end)
    end)
...
local FarmSec  = FarmPage:Section("Kontrol Farm", "Left")
local FarmInfo = FarmPage:Section("Konfigurasi", "Right")

FarmSec:Label("Flow: Beli → Tanam → Tunggu → Harvest → Jual")

FarmSec:Toggle("Full Auto Farm", "AutoFarmToggle", false,
    "Jalankan semua siklus farming otomatis",
    function(v)
        _G.AutoFarm = v
        if not v then notif("Auto Farm", "Dihentikan", 2); return end
        if #LahanCache == 0 then scanLahan() end
        if #LahanCache == 0 then
            notif("⚠️", "Scan Lahan dulu! (tab Teleport)", 5)
...
FarmSec:Toggle("Auto Tanam Loop", "AutoTanamToggle", false,
    "Tanam terus menerus ke semua lahan",
    function(v)
        _G.AutoTanam = v
        if v then
            task.spawn(function()
...
FarmSec:Toggle("Auto Mandi Tiap Siklus", "AutoMandiToggle", false,
    "Pergi mandi otomatis setiap selesai siklus",
    function(v) _G.AutoMandi = v end)

FarmSec:Button("⛔ STOP SEMUA", "Matikan semua fitur sekarang", stopSemua)

FarmInfo:Slider("Delay Tanam", "SliderTanam", 1, 30, 5,
    function(v) dTanam = v / 10 end,
    "Jeda antar tanam (x0.1 detik, default 5 = 0.5s)")

FarmInfo:Slider("Tunggu Panen (detik)", "SliderPanen", 10, 300, 60,
    function(v) waitPanen = v end,
    "Waktu tunggu sebelum mulai harvest")

FarmInfo:Label("Lihat console F9 untuk status live")
...
BibitLeft:Button("🛒 Beli Sekarang",
    "Beli bibit yang dipilih sejumlah slider",
    function()
        task.spawn(function()
            local ok, msg = beliBibit(selectedBibit, jumlahBeli)
...
BibitLeft:Button("📋 Cek Stok Bibit",
    "Lihat semua bibit dan jumlah di toko",
    function()
        task.spawn(function()
            ...
            for _, s in ipairs(data.Seeds) do
                txt = txt..(s.Locked and "[LOCK] " or "[OK]  ")
...
JualLeft:Label("RequestSell SELL — Cobalt Confirmed ✅")

JualLeft:Button("💰 Jual Semua Sekarang",
    "Jual semua item yang ada di inventori",
    function()
        task.spawn(function()
            local ok, msg = jualSemua()
...
JualLeft:Button("📋 Lihat Inventory",
    "Cek semua item yang bisa dijual beserta stok",
    function()
        task.spawn(function()
            local data = getInventoryJual()
...
for _, item in ipairs(ITEM_LIST) do
    ...
    JualRight:Button("Jual "..it.name.."  "..it.price.."💰/pcs",
        "Jual semua stok "..it.name.." sekarang",
        function()
            task.spawn(function()
                local data = getInventoryJual()
                local owned = 0
...
local HarvRight = HarvPage:Section("Info & Tips", "Right")

HarvLeft:Toggle("Auto Harvest", "AutoHarvToggle", false,
    "TP ke tiap tanaman matang dan panen otomatis",
    function(v)
        _G.AutoHarvest = v
        if v then
            HarvestLoop = task.spawn(function()
                while _G.AutoHarvest do
                    local h = autoHarvestAll()
                    if h > 0 then notif("🌾 Harvest", h.." tanaman dipanen", 2) end
...
HarvLeft:Button("🌾 Harvest Sekali Sekarang",
    "Langsung harvest semua tanaman satu kali",
    function()
        task.spawn(function()
            notif("🌾 Mulai Harvest...", "Memproses semua lahan", 2)
            local h = autoHarvestAll()
            notif("✅ Harvest Selesai", h>0 and h.." tanaman" or "Tidak ada tanaman", 3)
        end)
    end)

HarvLeft:Button("🗑 Reset Counter Panen",
    "Reset hitungan total panen ke 0",
    function() harvestCount = 0; notif("Reset", "Counter panen di-reset", 2) end)

HarvRight:Paragraph("Cara Kerja",
    "Script TP ke tiap tanaman yang punya ProximityPrompt lalu fireproximityprompt() otomatis. Setelah semua selesai, karakter balik ke posisi awal.")

HarvRight:Paragraph("Tips",
    "Aktifkan Penangkal Petir saat harvest agar tidak terganggu petir. Interval 10-15 detik cukup untuk kebanyakan lahan.")
...
TpLeft:Label("5 NPC Confirmed — Y otomatis dari workspace")
...
TpLeft:Button("🚿 TP ke Tempat Mandi",
    string.format("X=%d  Z=%d", MANDI.x, MANDI.z),
    goMandi)
...
TpLeft:Button("🔍 Scan Lahan",
    "Refresh cache posisi lahan sawah",
    function()
        LahanCacheTime = 0; local l = scanLahan()
        notif("Scan Lahan", #l.." plot ditemukan", 3)
    end)

TpLeft:Button("📍 Print Posisi Saya",
    "Cetak koordinat karakter sekarang ke console",
    function()
        local pos = getPos()
        if pos then
            notif("📍 Posisi Kamu",
...
for i = 1, 5 do
    local idx = i
    TpRight:Button("💾 Save Slot "..idx,
        "Simpan posisi sekarang ke slot "..idx,
        function()
            local cf = getCF()
    ...
    TpRight:Button("🚀 Load Slot "..idx,
        "Teleport ke posisi tersimpan di slot "..idx,
        function()
...
PetirLeft:Toggle("Penangkal Petir AKTIF", "PenangkalToggle", false,
    "Aktifkan flee + HP lock saat petir menyambar",
    function(v)
        _G.PenangkalPetir = v
        if v then
            ...
            notif("⚡ Penangkal Petir", "OFF", 2)
        end
    end)

PetirLeft:Paragraph("3 Lapis Perlindungan",
    "① HP dikunci ke MaxHP selama 7 detik\n② TP kabur (titik aman atau naik Y+350)\n③ Balik ke posisi asal setelah 5 detik")

PetirLeft:Paragraph("Mode Naik Awan",
    "Jika titik aman belum diset, karakter naik ke Y+350 — server tidak bisa reach di ketinggian itu. Sangat efektif!")

PetirLeft:Button("⚡ Test Petir Sekarang",
    "Simulasi flee petir secara manual untuk test",
    function()
        if not getRoot() then notif("❌","Karakter tidak ada",3); return end
        task.spawn(function() notif("⚡ Test", "Simulasi dimulai...", 2); fleePetir() end)
    end)

PetirLeft:Button("🗑 Reset Counter Petir",
    "Reset hitungan tangkalan ke 0",
    function() lightningHits = 0; notif("Reset", "Counter petir di-reset", 2) end)

PetirRight:Label("Titik aman = lokasi di dalam bangunan")

PetirRight:Button("📍 Set Titik Aman (posisi saya sekarang)",
    "Simpan posisi sekarang sebagai titik flee saat petir",
    function()
        local cf = getCF()
        if cf then
            fleePos = cf
...
PetirRight:Button("🗑 Hapus Titik Aman (pakai naik awan)",
    "Kembali ke mode naik Y+350 saat petir",
    function()
        fleePos = nil
        notif("Mode Naik Awan", "Titik aman dihapus", 3)
    end)

PetirRight:Paragraph("Status",
    "Toggle ON lalu test dengan tombol kiri.\nStatus kabur dan tangkalan lihat di console F9.")
...
SetLeft:Toggle("Anti AFK", "AntAFKToggle", false,
    "Jump kecil tiap 2 menit agar tidak di-kick server",
    function(v)
        _G.AntiAFK = v
...
SetLeft:Toggle("Notif Level Up", "NLvUpToggle", true,
    "Tampilkan notifikasi saat karakter level naik",
    function(v) _G.NotifLevelUp = v end)

SetLeft:Toggle("Auto Confirm", "AutoConfirmToggle", false,
    "Auto klik tombol konfirmasi dari server",
    function(v) _G.AutoConfirm = v; notif("Auto Confirm", v and "ON" or "OFF", 2) end)

SetLeft:Button("⛔ STOP SEMUA",
    "Matikan seluruh fitur auto dalam satu klik",
    stopSemua)

SetLeft:Button("🔄 Reset Session Stats",
    "Reset semua hitungan statistik sesi ke nol",
    function()
        totalEarned=0; harvestCount=0; SiklusCount=0; levelUpCount=0; lightningHits=0
        notif("Reset Stats", "Semua statistik sesi di-reset", 2)
    end)

SetRight:Paragraph("Indo Farmer v17.0",
    "XKID HUB — Aurora UI Edition\nSawah Indo — Cobalt Confirmed")

SetRight:Paragraph("Remote Confirmed ✅",
    "RequestShop → BUY, GET_LIST\nRequestSell → GET_LIST, SELL\nPlantCrop → FireServer(Vector3)\nHarvestCrop → OnClientEvent")
...
-- Status loop ke console
task.spawn(function()
    while _G.ScriptRunning do
        task.wait(15)
        print(string.format(
            "[ XKID ] Farm=%s Harv=%s Sell=%s Petir=%s | Lv=%d Coins=%d | Siklus=%d Panen=%d",
            tostring(_G.AutoFarm), tostring(_G.AutoHarvest),
            tostring(_G.AutoSell), tostring(_G.PenangkalPetir),
            PlayerData.Level, PlayerData.Coins, SiklusCount, harvestCount
        ))
    end
end)

Library:Notification("Indo Farmer v17", "Welcome, "..LocalPlayer.Name.."!", 5)
Library:ConfigSystem(Win)

print("╔══════════════════════════════════════════╗")
print("║   🌾  INDO FARMER v17.0  — XKID HUB    ║")
print("║   Aurora UI  ·  Cobalt Confirmed        ║")
