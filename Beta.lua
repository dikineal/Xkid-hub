...
RunService.Heartbeat:Connect(function() local r=getRoot() if r then lastCFrame=r.CFrame end end)
...
local function buildAreaData()
    ...
    for _,idx in ipairs(AREA_INDICES) do
        ...
        if obj then
            local parts={}
            if obj:IsA("BasePart") then table.insert(parts,obj) else for _,p in ipairs(obj:GetChildren()) do if p:IsA("BasePart") then table.insert(parts,p) end end if #parts==0 then for _,p in ipairs(obj:GetDescendants()) do if p:IsA("BasePart") and p.CanCollide then table.insert(parts,p) end end end end
...
FarmL:Button("🌱 Mulai Tanam","Tanam instant",function()
    if not Farm.selectedArea or not Farm.selectedCrop then notify("Error","Pilih area & tanaman!",3) return end
    local plotList=AREA_PLOTS[Farm.selectedArea]
    if not plotList then notify("Error","Area tidak ada!",2) return end
...
FarmM:Toggle("Auto Farm (Full Cycle)","autoFarm",false,"Beli→Tanam→Tunggu→Panen",function(v)
    ...
    if v then
        if not Farm.selectedArea or not Farm.selectedCrop then notify("Error","Pilih area & crop!",2) Farm.autoFarmActive=false return end
        Farm.autoFarmTask=task.spawn(function()
            while Farm.autoFarmActive do
                if Farm.autoBeli then beliBibit(Farm.selectedCrop,1) task.wait(0.5) end
                local plotList=AREA_PLOTS[Farm.selectedArea]
                local filtered=filterByPola(plotList,Farm.selectedPola,Farm.jumlahTanam)
                local planted=tanamPlots(Farm.selectedArea,Farm.selectedCrop,Farm.selectedPola,Farm.jumlahTanam)
        ...
        if Farm.autoFarmTask then pcall(function() task.cancel(Farm.autoFarmTask) end) Farm.autoFarmTask=nil end
...
FarmM:Button("💰 Panen Semua","Panen dari semua plot",function()
    if not Farm.selectedArea then notify("Error","Pilih area!",2) return end
    local allParts=AREA_PARTS[Farm.selectedArea]
    if not allParts then notify("Error","Tidak ada plot!",2) return end
...
ShopL:Button("🛒 Beli Sekarang","Execute beli",function()
    if not shopCrop then notify("Error","Pilih bibit!",2) return end
    if beliBibit(shopCrop,shopQty) then notify("Beli","✅ "..shopQty.." bibit dibeli",2) else notify("Beli","❌ Gagal",2) end
end)

for _,c in ipairs(CROPS) do
    ShopR:Button(c.icon.." "..c.name,"Harga: "..c.price.." | Jual: "..c.sell,function()
        if beliBibit(c,1) then notify("Beli","✅ Bibit "..c.name.." dibeli",1) end
    end)
end
...
for _,areaName in ipairs(AREA_NAMES) do
    TeleL:Button("TP: "..areaName,"Teleport instant",function()
        ...
        if plots and #plots>0 then local r=getRoot() if r then r.CFrame=plots[1].part.CFrame+Vector3.new(0,3,0) notify("TP","✅ Ke "..areaName,1) end end
    end)
end

TeleL:Button("🏠 Spawn","Ke spawn area",function()
    local r=getRoot()
    if r then r.CFrame=CFrame.new(0,50,0) notify("TP","✅ Ke spawn",1) end
end)
    ...
    if p~=LP then
        TeleR:Button("👤 "..p.Name,"TP instant",function()
            if p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                local r=getRoot()
                if r then r.CFrame=p.Character.HumanoidRootPart.CFrame+Vector3.new(0,3,0) notify("TP","✅ Ke "..p.Name,1) end
            end
        end)
    end
end
...
SecR:Toggle("Auto Respawn","autoResp",false,"Otomatis setelah mati",function(v)
    ...
    if v then startAutoRespawn() notify("Auto Respawn","✅ ON",1) else stopAutoRespawn() notify("Auto Respawn","❌ OFF",1) end
end)
SecR:Button("📍 Save Posisi","Simpan posisi saat ini",function()
    local root=getRoot()
    if root then Respawn.savedPosition=root.CFrame notify("Posisi","✅ Tersimpan",1) end
end)
...
SetL:Toggle("Auto Fishing","autoFish",false,"Auto cast loop 31.6s",function(v)
    Fish.autoOn=v
    if v then
        if not Fish.rodEquipped then if not equipRod() then Fish.autoOn=false return end end
        Fish.fishTask=task.spawn(function()
            while Fish.autoOn do
                local ok=pcall(castOnce)
                if ok then Fish.errorCount=0 task.wait(1) else Fish.errorCount=Fish.errorCount+1 if Fish.errorCount>=3 then notify("Fishing","Auto stopped - errors",3) Fish.autoOn=false break end task.wait(5) end
            end
        end)
        notify("Fishing","✅ ON",2)
    else
...
SetL:Button("🎣 Cast 1x","Cast kail sekali",function()
    if not Fish.rodEquipped then if not equipRod() then return end end
    if castOnce() then notify("Fishing","✅ 1x cast selesai",1) end
end)
...
SetR:Button("📋 Lihat Log","5 log terakhir",function()
    if #logLines==0 then notify("Log","Kosong",1) return end
    local txt=""
...
SetR:Paragraph("ℹ️ XKID HUB v6.0","✅ Full Farming\n✅ Manual+Auto\n✅ Fishing v2\n✅ Complete")

local _totalPl=0
for _,v in pairs(AREA_PARTS or {}) do _totalPl=_totalPl+#v end
if _totalPl>0 then
    notify("✅ XKID HUB v6.0 Ready",#AREA_NAMES.." area | ".._totalPl.." plot",5)
else
    notify("⚠️ Warning","Plot tidak ditemukan!",5)
end

Library:Notification("XKID HUB v6.0","Manual Farming · Auto Farming · Fishing v2",6)
