...
║  [MODIFIED] Removed Farming & Fishing & Remotes          ║
╚═══════════════════════════════════════════════════════════╝
]]
...
local function _mkPlBill(p)
    ...
    bill.Size = UDim2.new(0, 100, 0, 24); bill.AlwaysOnTop = true; bill.StudsOffset = Vector3.new(0, 2, 0)
    local lbl = Instance.new("TextLabel", bill)
    lbl.Size = UDim2.new(1, 0, 1, 0); lbl.TextColor3 = Color3.fromRGB(255, 230, 80); lbl.TextScaled = true; lbl.BackgroundTransparency = 1; lbl.Font = Enum.Font.GothamBold
...
-- TAB TELEPORT
local TPR = T_TP:Page("Teleport", "map-pin"):Section("Lokasi & Player", "Left")
TPR:TextBox("Cari Player", "tpP", "", function(v)
    local best = nil; for _, p in pairs(Players:GetPlayers()) do
        if p ~= LP and p.Name:lower():sub(1, #v) == v:lower() then best = p; break end
    end
    if best and best.Character then getRoot().CFrame = best.Character.HumanoidRootPart.CFrame end
end)
for i = 1, 5 do
    TPR:Button("Save Slot " .. i, "Simpan posisi", function() SavedLoc[i] = getRoot().CFrame end)
    TPR:Button("Load Slot " .. i, "Teleport", function() if SavedLoc[i] then getRoot().CFrame = SavedLoc[i] end end)
end

-- TAB PLAYER
local PL = T_Pl:Page("Movement", "user"):Section("Speed & Fly", "Left")
PL:Slider("Walk Speed", "ws", 16, 500, 16, function(v) Move.speed = v; getHum().WalkSpeed = v end)
PL:Toggle("Fly", "fly", false, "Terbang", function(v) if v then startFly() else stopFly() end end)
PL:Toggle("NoClip", "nc", false, "Tembus Dinding", function(v) setNoclip(v) end)
PL:Toggle("ESP Player", "esp", false, "Lihat Player", function(v)
    ESPPl.active = v
    if v then 
...
-- TAB SECURITY
local SL = T_Sec:Page("Security", "shield"):Section("Protection", "Left")
...
notify("XKID HUB", "Utility Edition Ready!")
