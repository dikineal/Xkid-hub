...
========================
  ...
  • Network (Auto Rejoin, Server Hop)
  ...
  FIXED:
  • Live Monitor ping & FPS real-time
  • NoClip via Heartbeat (bukan Stepped)
  • Soft Fling via Heartbeat tiap frame
...
local function fastRespawn()
    ...
    notify("Respawn", "Respawn aman 💨", 1.5)
    ...
    task.spawn(function()
        ...
        conn = LP.CharacterAdded:Connect(function(newChar)
            ...
            if newHrp and newHum then
                ...
                notify("Sukses", "Respawn di tempat berhasil!", 2)
            end
...
local function refreshCharacter()
    ...
    if not char or not hrp then notify("Error", "Karakter gak ada!", 2); return end
    State.Avatar.isRefreshing = true
    notify("Refresh", "Refresh karakter 🔁", 1.5)
    ...
    task.spawn(function()
        ...
        conn = LP.CharacterAdded:Connect(function(newChar)
            ...
            if newHrp and newHum then
                ...
                notify("Sukses", "Refresh di tempat berhasil!", 2)
            else
                notify("Error", "Gagal refresh!", 2)
            end
...
local function initPlayerCache(player)
    ...
    pcall(function()
        ...
        cache.texts.Center = true; cache.texts.Outline = true
        cache.texts.Font   = 2;    cache.texts.Size    = 13; cache.texts.ZIndex = 2
    ...
    RS:BindToRenderStep("XKIDSpec", Enum.RenderPriority.Camera.Value + 1, function()
        ...
        pcall(function()
            ...
            or not Spec.target.Character:FindFirstChild("HumanoidRootPart") then
                notify("Spectate", "Target gak valid!", 2)
...
local function logMsg(speakerName, msg)
    ...
    if State.Utility.chatTarget
    and State.Utility.chatTarget.Name ~= speakerName
    and State.Utility.chatTarget.DisplayName ~= speakerName then return end
...
local Window = WindUI:CreateWindow({
    ...
    User = {
        ...
        Anonymous = false,
        UserId    = LP.UserId,
        Callback  = function() notify("XKID", "Dibuat oleh @WTF.XKID", 3) end,
    },
})
...
secMov:Slider({ Title = "Walk Speed", Step = 1, Value = { Min = 16, Max = 500, Default = 16 },
    Callback = function(v) State.Move.ws = v; if getHum() then getHum().WalkSpeed = v end; notify("Gerak", "Speed: "..v, 1) end })
secMov:Slider({ Title = "Jump Power", Step = 1, Value = { Min = 50, Max = 500, Default = 50 },
    Callback = function(v) State.Move.jp = v; local h = getHum(); if h then h.UseJumpPower = true; h.JumpPower = v end; notify("Gerak", "Jump: "..v, 1) end })
secMov:Toggle({ Title = "🦘 Infinite Jump", Value = false, Callback = function(v)
    ...
    notify("Gerak", v and "Inf Jump 🦘" or "Inf Jump mati", 2)
end})
...
secAbi:Toggle({ Title = "💫 Soft Fling", Value = false, Callback = function(v)
    ...
    notify("Fling", v and "Soft Fling siap 💫" or "Fling mati", 2)
end})
...
local secClickTP = T_TP:Section({ Title = "🖱️ Click Teleport", Opened = true })
...
local tpDropdown = secTP:Dropdown({ Title = "Daftar Player", Values = pDropOpts,
...
for i = 1, 3 do
    ...
    secLoc:Button({ Title = "💾 Save Slot "..idx, Callback = function()
        ...
        SavedLocs[idx] = r.CFrame; notify("Slot", "Slot "..idx.." tersimpan!", 2)
    end})
    secLoc:Button({ Title = "📍 Load Slot "..idx, Callback = function()
        if not SavedLocs[idx] then notify("Error", "Slot kosong!", 2); return end
        local r = getRoot(); if not r then return end
        r.CFrame = SavedLocs[idx]; notify("Slot", "Ke slot "..idx.."!", 2)
    end})
end
...
T_CAM:Section({ Title = "🔍 Camera Zoom", Opened = true }):Toggle({ Title = "Max Zoom Out", Value = false,
    Callback = function(v)
        pcall(function() LP.CameraMaxZoomDistance = v and 100000 or 400 end)
        notify("Camera", v and "Zoom jauh aktif 🔭" or "Zoom normal", 2)
    end})
...
secFC:Slider({ Title = "Speed", Step = 0.5, Value = { Min = 1, Max = 20, Default = 3 },
...
secSP:Button({ Title = "🔄 Refresh", Callback = function()
    ...
    notify("Spectate", "List diperbarui!", 2)
end})
secSP:Toggle({ Title = "Spectate ON", Value = false, Callback = function(v)
    ...
    if v then
        ...
        notify("Spectate", "Ngawasin "..Spec.target.DisplayName.." 👀", 2)
    else
        stopSpecLoop(); stopSpecCapture()
        Cam.CameraType = Enum.CameraType.Custom; Cam.FieldOfView = Spec.origFov
        notify("Spectate", "Selesai ✌️", 2)
    end
end})
secSP:Toggle({ Title = "🎯 First Person", Value = false,
    Callback = function(v) Spec.mode = v and "first" or "third" end })
secSP:Slider({ Title = "Jarak", Step = 1, Value = { Min = 3, Max = 30, Default = 8 },
...
local chatTargetDrop = secChat:Dropdown({ Title = "Pilih Target", Values = getDisplayNames(), Callback = function(v)
    ...
    if p then
        ...
        notify("Chat Log", "Ngikutin "..p.DisplayName, 2)
    end
end})
secChat:Button({ Title = "🔄 Refresh", Callback = function()
    pcall(function() chatTargetDrop:Refresh(getDisplayNames(), true) end)
    notify("Chat Log", "List diupdate", 2)
end})
secChat:Button({ Title = "🗑️ Clear Log", Callback = function()
    State.Utility.chatHistory = {}
    pcall(function() chatLogPanel:SetDesc("Menunggu chat...") end)
    notify("Chat Log", "Log dibersihkan", 2)
end})

local secMisc = T_UTIL:Section({ Title = "🛠️ Misc", Opened = true })
secMisc:Button({ Title = "📋 Copy JobID", Callback = function()
    pcall(function() setclipboard(game.JobId) end); notify("Utilitas", "JobID tersalin!", 2)
end})
secMisc:Slider({ Title = "FPS Capper", Step = 5, Value = { Min = 30, Max = 240, Default = 60 }, Callback = function(v)
    pcall(function() setfpscap(v) end); notify("Utilitas", "FPS Cap: "..v, 2)
end})
    ...
    if v then
        State.Security.voidConn = TrackC(RS.Heartbeat:Connect(function()
            ...
            if hrp and hrp.Position.Y <= workspace.FallenPartsDestroyHeight + 50 then
                ...
                notify("Anti Void", "Terselamatkan!", 2)
            end
        end))
    ...
    if v then
        State.Security.arConn = TrackC(CoreGui.RobloxPromptGui.promptOverlay.ChildAdded:Connect(function(child)
            if child.Name == "ErrorPrompt" then
                notify("Auto Rejoin", "Kena kick, rejoin...", 3)
...
secSrv:Button({ Title = "🔁 Rejoin", Callback = function()
    notify("Rejoin", "Masuk ulang...", 2)
...
T_SEC:Section({ Title = "🔒 Camera Lock", Opened = true })
    :Toggle({ Title = "Shift Lock", Value = false, Callback = function(v) toggleShiftLock(v) end })
...
secCfg:Button({ Title = "🔄 Refresh List", Callback = function()
    pcall(function() configDrop:Refresh(getConfigList(), true) end); notify("Config", "List diupdate!", 2)
end})
...
secTheme:Toggle({ Title = "Acrylic", Value = true, Callback = function()
    pcall(function() WindUI:ToggleAcrylic(not WindUI.Window.Acrylic) end)
end})
secTheme:Toggle({ Title = "Transparent", Value = true, Callback = function(s)
...
notify("XKID", "Script siap 💎 | All bugs fixed", 3)
print("✅ @WTF.XKID Script Loaded | All Bugs Fixed Edition")
