...
FarmL:Toggle("🌾 Auto Panen", "AutoPanenTog", false,
    ...
    function(v)
        ...
        if v then
            ...
            Library:Notification("🌾 Auto Panen", "✅ ON — tiap 5 detik", 2)
        else
...
ManR:Button("🌾 Panen Sekarang", "Harvest semua lahan sekarang",
    function()
        task.spawn(function()
            ...
            Library:Notification("✅ Panen",
                string.format("%d request\nPosisi matang: %d\nServer proses jika matang",
                    n, #matureCrops), 4)
        end)
    end)

ManR:Button("🔍 Scan Posisi Tanaman Matang", "Lihat posisi tanaman yang sudah matang",
    function()
        if #matureCrops == 0 then
            Library:Notification("🔍 Scan",
                "Belum ada data posisi matang\n\nAktifkan Monitor Panen dulu\nlalu tunggu tanaman tumbuh", 5)
            return
        end
        local text = #matureCrops .. " posisi tanaman matang:\n\n"
        for i, mc in ipairs(matureCrops) do
            if mc.cropPos then
                text = text .. string.format("[%d] %s X=%.0f Z=%.0f\n",
                    i, mc.cropName, mc.cropPos.X, mc.cropPos.Z)
            end
        end
        Library:Notification("🌾 Posisi Matang", text, 12)
    end)

ManR:Button("🗑️ Reset Data Matang", "Hapus cache posisi tanaman matang",
    function()
        matureCrops = {}
        Library:Notification("🗑️", "Cache direset", 2)
    end)
