...
-- ══════════════════════════════════════════════════════════════
--  AUTO-CLEANUP (Mencegah Lag saat Re-Execute)
-- ══════════════════════════════════════════════════════════════
...
local State = {
    ...
    Ghost    = { active = false }, -- Fitur Baru
    Chat     = { bypass = false }, -- Fitur Baru
    ESP = {
        active          = false,
        cache           = {},
        boxMode         = "Corner",
...
-- ══════════════════════════════════════════════════════════════
--  CHAT COMMANDS & BYPASS (Fitur Baru)
-- ══════════════════════════════════════════════════════════════
LP.Chatted:Connect(function(msg)
    local lowMsg = string.lower(msg)
    
    if lowMsg == ";re" or lowMsg == "/re" then
...
local function toggleFly(v)
    ...
    State.Fly.bg.P=50000 -- Diperbesar agar kaku dan tidak meluncur licin (Linear)
    ...
    RS:BindToRenderStep("XKIDFly", Enum.RenderPriority.Camera.Value+1, function()
        ...
        if move.Magnitude > 0 then 
            ...
            State.Fly.bv.Velocity = Vector3.zero -- Berhenti langsung
        end
...
local FC = {
    ...
    savedCF=nil, damping=0.40, accel=0.50, -- Responsif
}
...
local Window = WindUI:CreateWindow({
    ...
    Acrylic     = true,
    Transparent = true,
    Size        = UDim2.fromOffset(580, 420), -- UKURAN COMPACT (Hanya ubah ini)
...
-- ══════════════════════════════════════════════════════════════
--  TAB: PLAYER (100% UTUH + GHOST MODE)
-- ══════════════════════════════════════════════════════════════
...
-- GHOST MODE (FITUR BARU)
secAbi:Toggle({
    Title    = "Ghost Mode (Desync)",
    Desc     = "Badan diam di server, kamu bebas keliling",
    Value    = false,
    Callback = function(v)
        ...
        if v and hrp then
            hrp.Anchored = true
            notify("Ghost Mode", "Aktif! Tubuh aslimu terkunci di server.", 3)
        elseif hrp then
            hrp.Anchored = false
            notify("Ghost Mode", "Nonaktif! Tubuh kembali sinkron.", 2)
        end
    end,
})
...
secGfx:Button({ Title="📉  Low  (Lv3)",     Callback=function() setGfx(Enum.QualityLevel.Level03); notify("Graphics","Low — Lv3",2)   end })
secGfx:Button({ Title="📊  Medium  (Lv5)",  Callback=function() setGfx(Enum.QualityLevel.Level05); notify("Graphics","Medium — Lv5",2) end })
secGfx:Button({ Title="📈  High  (Lv8)",    Callback=function() setGfx(Enum.QualityLevel.Level08); notify("Graphics","High — Lv8",2)  end })
...
-- ══════════════════════════════════════════════════════════════
--  TAB: SECURITY (100% UTUH + MEMORY PURGE)
-- ══════════════════════════════════════════════════════════════
...
secProt:Toggle({
    Title    = "Chat Bypass Filter",
    Desc     = "Kirim chat tanpa sensor (bypass lokal)",
    Value    = false,
    Callback = function(v) State.Chat.bypass = v end,
})
...
secPerf:Button({ Title="Memory Purge", Desc="Bersihkan RAM saat ngelag", Callback=function() collectgarbage("collect"); notify("System", "Memory Dibersihkan!", 2) end })
...
task.spawn(function()
    while true do
        ...
        if #fpsSamples > 0 then
            ...
            if fpsLabel then
                pcall(function()
                    fpsLabel:SetDesc(color.."  "..fps.." FPS    ["..bar.."]")
                end)
            end
        end
    end
...
-- THEME SETTINGS (YANG HILANG SEBELUMNYA SUDAH KEMBALI)
