...
-- ══════════════════════════════════════════════════════════════
--  0. AUTO CLEANUP & MEMORY PLUG (PRO 2026)
-- ══════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════
--  CHAT COMMAND (:re) & ANTI-FILTER HOOK
-- ══════════════════════════════════════════════════════════════
    ...
    if cmd == ":re" or cmd == "/re" then
        ...
        if r and h then
            ...
            task.spawn(function()
                ...
                if hrp then hrp.CFrame = savedCF; notify("Command", "✅ Auto-Respawn Executed", 2) end
            end)
        end
    end
end))
...
local function toggleFly(v)
    if not v then
        ...
        local hum = getHum()
        if hum then
            hum.PlatformStand=false; hum:ChangeState(Enum.HumanoidStateType.GettingUp); hum.WalkSpeed=State.Move.ws; hum.UseJumpPower=true; hum.JumpPower=State.Move.jp
        end
        notify("Fly","✈  Fly OFF")
        return
    end
    ...
    RS:BindToRenderStep("XKIDFly", Enum.RenderPriority.Camera.Value+1, function()
        ...
        if onMobile then move = camCF.LookVector*(-flyJoy.Y)*spd + camCF.RightVector*flyJoy.X*spd else
...
local function startFCCapture()
    ...
    table.insert(fcConns, UIS.TouchMoved:Connect(function(inp)
        if inp==fcRotT and fcRotLast then
            ...
            FC.pitchDeg = math.clamp(FC.pitchDeg-(inp.Position.Y-fcRotLast.Y)*FC.sens,-80,80); fcRotLast=inp.Position
        end
...
local function stopFCCapture()
    ...
    fcConns={}; fcRotT=nil; fcMoveT=nil; fcMoveSt=nil; fcRotLast=nil; fcJoy=Vector2.zero; FC._mouseRot=false; FC._keys={}; UIS.MouseBehavior=Enum.MouseBehavior.Default
end
...
local function startFCLoop()
    RS:BindToRenderStep("XKIDFreecam", Enum.RenderPriority.Camera.Value+1, function(dt)
        ...
        FC.vel = FC.vel:Lerp(dv, FC.accel*dt*60); FC.vel = FC.vel*(FC.damping^(dt*60)); FC.pos = FC.pos+FC.vel*dt
        Cam.CFrame = CFrame.new(FC.pos) * CFrame.Angles(0,math.rad(FC.yawDeg),0) * CFrame.Angles(math.rad(FC.pitchDeg),0,0)
        local hrp=getRoot(); local hum=getHum()
        if hrp and not hrp.Anchored then hrp.Anchored=true end
        if hum then if hum:GetState()~=Enum.HumanoidStateType.Physics then hum:ChangeState(Enum.HumanoidStateType.Physics) end; hum.WalkSpeed=0; hum.JumpPower=0 end
    end)
end
...
local Window = WindUI:CreateWindow({
    Title       = "XKID SCRIPT", Author = "by XKID", Folder = "XKIDScript", Icon = "shield", Theme = "Rose", Acrylic = true, Transparent = true,
    Size        = UDim2.fromOffset(700, 480), MinSize = Vector2.new(560, 380), MaxSize = Vector2.new(860, 580), ToggleKey = Enum.KeyCode.RightShift,
    Resizable   = true, AutoScale = true, NewElements = true, SideBarWidth= 200, Topbar = { Height = 44, ButtonsType = "Default" },
...
secTP:Button({ Title="Teleport", Desc="Teleport ke player yang dicari", Callback=function()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LP then
            ...
            if (string.find(nl,tl) or string.find(dl,tl)) and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                getRoot().CFrame = p.Character.HumanoidRootPart.CFrame; notify("Teleport","✅  TP ke "..p.DisplayName, 2); return
            end
        end
    end
...
local SavedLocs = {}
for i = 1, 5 do secLoc:Button({ Title="Save Slot "..i, Desc="Simpan posisi sekarang", Callback=function() local r=getRoot(); if not r then return end; SavedLocs[i]=r.CFrame; notify("Location","💾 Slot "..i.." tersimpan!", 2) end }) end
for i = 1, 5 do secLoc:Button({ Title="Load Slot "..i, Desc="Teleport ke lokasi slot "..i, Callback=function() if not SavedLocs[i] then return end; local r=getRoot(); if not r then return end; r.CFrame=SavedLocs[i]; notify("Location","📍 Teleport ke Slot "..i, 2) end }) end
...
secMov:Button({ Title="Refresh POV", Desc="Reset kamera & karakter", Callback=function() local r=getRoot(); local h=getHum(); if not r or not h then return end; Cam.CameraType=Enum.CameraType.Custom; task.wait(0.05); Cam.CameraType=Enum.CameraType.Scriptable; task.wait(0.05); Cam.CameraType=Enum.CameraType.Custom; pcall(function() h:ChangeState(Enum.HumanoidStateType.GettingUp) end); notify("Refresh","✅  POV & kamera di-reset!", 2) end })
secMov:Slider({ Title="Walk Speed", Desc="Default: 16", Step=1, Value={Min=16,Max=500,Default=16}, Callback=function(v) State.Move.ws=v; if getHum() then getHum().WalkSpeed=State.Move.ws end end })
secMov:Slider({ Title="Jump Power", Desc="Default: 50", Step=1, Value={Min=50,Max=500,Default=50}, Callback=function(v) State.Move.jp=v; local hum=getHum(); if hum then hum.UseJumpPower=true; hum.JumpPower=State.Move.jp end end })
...
secAbi:Slider({ Title="Fly Speed", Desc="Default: 60", Step=1, Value={Min=10,Max=300,Default=60}, Callback=function(v) State.Move.flyS=v end })
...
secLock:Toggle({ Title="Shift Lock", Desc="Karakter selalu hadap kamera", Value=false, Callback=function(v)
    local hum=getHum(); local hrp=getRoot(); if not hum or not hrp then return end
    if v then
...
secFC:Toggle({ Title="Freecam", Desc="PC: RMB rotate  |  Mobile: Kiri gerak / Kanan rotate", Value=false, Callback=function(v)
    ...
    if v then
        ...
        FC._keys={}; FC._mouseRot=false; local hrp=getRoot(); local hum=getHum()
        ...
        startFCCapture(); startFCLoop(); notify("Freecam","🎬  ON", 3)
    else
        stopFCLoop(); stopFCCapture(); local hrp=getRoot(); local hum=getHum()
...
local function applyPreset(fov,spd,clock,bright,fogEnd,fogR,fogG,fogB,ambR,ambG,ambB,density,offset,glare,halo,gfxLevel)
    Cam.FieldOfView=fov; FC.speed=spd; Lighting.ClockTime=clock; Lighting.Brightness=bright; Lighting.FogEnd=fogEnd; Lighting.FogColor=Color3.fromRGB(fogR,fogG,fogB); Lighting.Ambient=Color3.fromRGB(ambR,ambG,ambB)
    local atm=Lighting:FindFirstChildOfClass("Atmosphere") or Instance.new("Atmosphere",Lighting); atm.Density=density; atm.Offset=offset; atm.Glare=glare; atm.Halo=halo
    pcall(function() settings().Rendering.QualityLevel=gfxLevel end)
end
secPre:Button({ Title="☀  Cinematic Day", Callback=function() applyPreset(50,3,14,2,10000,200,220,255,120,120,120,0.05,0.1,0.3,0.2,Enum.QualityLevel.Level10) end })
secPre:Button({ Title="↺  Reset Default", Callback=function() applyPreset(70,5,14,1,100000,191,191,191,70,70,70,0.35,0,0,0.25,Enum.QualityLevel.Level05) end })
...
secFine:Slider({ Title="Brightness", Step=0.1, Value={Min=0,Max=5,Default=1}, Callback=function(v) Lighting.Brightness=v end })
secFine:Slider({ Title="Clock Time", Step=1, Value={Min=0,Max=24,Default=14}, Callback=function(v) Lighting.ClockTime=v end })
...
local function inJoystick(pos)
    ...
    if ctrl then
        ...
        if thumb then local ap=thumb.AbsolutePosition; local as=thumb.AbsoluteSize; if pos.X>=ap.X and pos.Y>=ap.Y and pos.X<=ap.X+as.X and pos.Y<=ap.Y+as.Y then return true end end
    end return false
end
local Spec = { active=false, target=nil, mode="third", dist=8, origFov=70, orbitYaw=0, orbitPitch=0, fpYaw=0, fpPitch=0 }
local specTM,specPinch,specPinchD = nil,{},nil; local specPan=Vector2.zero; local specConns={}
local function startSpecCapture()
    table.insert(specConns, UIS.InputBegan:Connect(function(inp,gp) if gp or not Spec.active then return end; if inp.UserInputType~=Enum.UserInputType.Touch then return end; if inJoystick(inp.Position) then return end; table.insert(specPinch,inp); specTM = #specPinch==1 and inp or nil end))
    table.insert(specConns, UIS.InputChanged:Connect(function(inp) if not Spec.active then return end; if inp.UserInputType~=Enum.UserInputType.Touch then return end; if #specPinch==1 and inp==specTM then specPan = specPan+Vector2.new(inp.Delta.X,inp.Delta.Y) elseif #specPinch>=2 then local d=(specPinch[1].Position-specPinch[2].Position).Magnitude; if specPinchD then local diff=d-specPinchD; Cam.FieldOfView=math.clamp(Cam.FieldOfView-diff*0.15,10,120); if Spec.mode=="third" then Spec.dist=math.clamp(Spec.dist-diff*0.03,3,30) end end; specPinchD=d end end))
...
local function startSpecLoop()
    RS:BindToRenderStep("XKIDSpec", Enum.RenderPriority.Camera.Value+1, function()
        ...
        if Spec.mode=="third" then
            ...
            local oCF = CFrame.new(hrp.Position) * CFrame.Angles(0,math.rad(-Spec.orbitYaw),0) * CFrame.Angles(math.rad(-Spec.orbitPitch),0,0) * CFrame.new(0,0,Spec.dist); Cam.CFrame=CFrame.new(oCF.Position, hrp.Position+Vector3.new(0,1,0))
        else
            local head=char:FindFirstChild("Head"); local origin=head and head.Position or hrp.Position+Vector3.new(0,1.5,0); Spec.fpYaw=Spec.fpYaw-pan.X*sens; Spec.fpPitch=math.clamp(Spec.fpPitch-pan.Y*sens,-85,85); Cam.CFrame=CFrame.new(origin) * CFrame.Angles(0,math.rad(Spec.fpYaw),0) * CFrame.Angles(math.rad(Spec.fpPitch),0,0)
        end
    end)
end
...
secSP:Toggle({ Title="First Person Mode", Value=false, Callback=function(v) Spec.mode=v and "first" or "third"; if v and Spec.target and Spec.target.Character then local _,ry,_=Cam.CFrame:ToEulerAnglesYXZ(); local rx=math.asin(Cam.CFrame.LookVector.Y); Spec.fpYaw=math.deg(ry); Spec.fpPitch=math.deg(rx) end end })
...
secCam:Button({ Title="Reset POV Camera", Callback=function() local r=getRoot(); local h=getHum(); if not r or not h then return end; Cam.CameraType=Enum.CameraType.Custom; task.wait(0.05); Cam.CameraType=Enum.CameraType.Scriptable; task.wait(0.05); Cam.CameraType=Enum.CameraType.Custom; pcall(function() h:ChangeState(Enum.HumanoidStateType.GettingUp) end) end })
secCam:Slider({ Title="FOV Zoom", Step=1, Value={Min=10,Max=120,Default=70}, Callback=function(v) Cam.FieldOfView=v end })
...
local function setGfx(level) local ok=pcall(function() settings().Rendering.QualityLevel=level end); if not ok then pcall(function() UserSettings():GetService("UserGameSettings").SavedQualityLevel=level end) end end
secGfx:Button({ Title="🥔  Potato  (Lv1)", Callback=function() setGfx(Enum.QualityLevel.Level01) end })
secGfx:Button({ Title="💎  Ultra  (Lv10)", Callback=function() setGfx(Enum.QualityLevel.Level10) end })

-- ══════════════════════════════════════════════════════════════
--  TAB: SECURITY (INCLUDING FULL ESP UI AND NEW ANTI-FILTER)
-- ══════════════════════════════════════════════════════════════
...
secProt:Toggle({ Title="Anti-AFK", Desc="Cegah kick saat AFK (Bypass mode)", Value=false, Callback=function(v)
    if v then
        ...
        pcall(function() for _,conn in pairs(getconnections(LP.Idled)) do conn:Disable() end end); notify("Anti-AFK","🛡  Bypass aktif!", 2)
    else
        if State.Security.afkConn then State.Security.afkConn:Disconnect(); State.Security.afkConn=nil end
        pcall(function() for _,conn in pairs(getconnections(LP.Idled)) do conn:Enable() end end); notify("Anti-AFK","❌  Nonaktif", 2)
    end
end})
...
secResp:Button({ Title="Fast Respawn", Callback=function() if not respawnLastPos then return end; local savedCF=respawnLastPos; local hum=getHum(); if not hum then return end; hum.Health=0; task.spawn(function() local char=LP.CharacterAdded:Wait(); task.wait(0.3); local hrp=char:WaitForChild("HumanoidRootPart",10); if hrp then hrp.CFrame=savedCF end end) end })

local secESP = T_SC:Section({ Title = "ESP Tracker", Opened = true })
secESP:Toggle({ Title="ESP  ON / OFF", Desc="Tampilkan posisi semua player", Value=false, Callback=function(v)
    State.ESP.active=v
    if not v then
...
secESP:Dropdown({ Title="ESP Box Color", Values={"Green","Red","Blue","White","Yellow","Purple"}, Value="Green", Callback=function(v)
    local colors = { Green=Color3.fromRGB(0,255,150), Red=Color3.fromRGB(255,50,50), Blue=Color3.fromRGB(0,150,255), White=Color3.fromRGB(255,255,255), Yellow=Color3.fromRGB(255,255,0), Purple=Color3.fromRGB(150,0,255) }
    local c = colors[v] or colors.Green; State.ESP.boxColor_N=c; State.ESP.tracerColor_N=c
end})
secESP:Dropdown({ Title="Text / Name Color", Values={"White","Green","Red","Blue","Yellow","Purple"}, Value="White", Callback=function(v)
    local colors = { White=Color3.fromRGB(255,255,255), Green=Color3.fromRGB(0,255,150), Red=Color3.fromRGB(255,50,50), Blue=Color3.fromRGB(0,150,255), Yellow=Color3.fromRGB(255,255,0), Purple=Color3.fromRGB(150,0,255) }
...
secESPPre:Button({ Title="Legit", Callback=function() State.ESP.boxMode="OFF"; State.ESP.tracerMode="OFF"; State.ESP.showDistance=true; State.ESP.showNickname=true; State.ESP.maxDrawDistance=250 end })
secESPPre:Button({ Title="Rage", Callback=function() State.ESP.boxMode="Corner"; State.ESP.tracerMode="Center"; State.ESP.showDistance=true; State.ESP.showNickname=true; State.ESP.maxDrawDistance=400 end })

local secPerf = T_SC:Section({ Title = "Performance", Opened = false })
secPerf:Button({ Title="Max FPS", Callback=function() if setfpscap then setfpscap(999) end end })
...
task.spawn(function()
    while true do
        ...
        if #fpsSamples > 0 then
            local avg = 0; for _,s in ipairs(fpsSamples) do avg=avg+s end; avg = avg / #fpsSamples
            local fps = math.floor(1/avg)
            local pct = math.clamp(fps/120, 0, 1); local filled = math.floor(pct * 10); local bar = ""
...
secTheme:Toggle({ Title="Acrylic", Value=true, Callback=function() local isOn=WindUI.Window.Acrylic; WindUI:ToggleAcrylic(not isOn) end })
...
WindUI:Notify({ Title = "XKID SCRIPT", Content = "Premium Edition V3 siap digunakan! 🚀", Duration = 5 })