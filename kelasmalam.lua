...
local State = {
    ...
    Teleport = { clickConn = nil, clickActive = false, lastTap = 0 },
    Security = { afkActive = true, shiftLock = false, shiftLockGyro = nil, antiLag = false, arConn = nil },
    Cinema = { hideUI = false, hideNametag = false, hideBubble = false, cachedGuis = {} },
...
local function findPlayerByDisplay(str) for _, p in pairs(Players:GetPlayers()) do if p.DisplayName == str or p.Name == str then return p end end; return nil end
...
local function isValidFileName(name) if not name or name == "" then return false end; return not name:match("[/\\:*?\"<>|]") end
...
local function startFreecamCapture()
    ...
    table.insert(fcConns, UIS.TouchMoved:Connect(function(inp) if inp == fcRotTouch and fcRotLast then local dx, dy = inp.Position.X - fcRotLast.X, inp.Position.Y - fcRotLast.Y; fcRotLast = inp.Position; I_YawVel = I_YawVel - dx * FC.sens * 80; I_PitchVel = I_PitchVel - dy * FC.sens * 80 end
        if inp == fcMoveTouch and fcMoveSt then local dx, dy = inp.Position.X - fcMoveSt.X, inp.Position.Y - fcMoveSt.Y; local function ad(v,d,m) if math.abs(v)<d then return 0 end; return math.clamp((v-math.sign(v)*d)/(m-d),-1,1) end; fcJoy = Vector2.new(ad(dx,15,70), ad(dy,15,70)) end end))
...
-- ⭐ FIXED: Freecam cleanup tanpa animasi
local function fullCleanupFreecam()
    ...
    if hrp then
        hrp.Anchored = false
        if FC.savedCF then
            hrp.CFrame = FC.savedCF; hrp.AssemblyLinearVelocity = Vector3.zero; hrp.AssemblyAngularVelocity = Vector3.zero
        end
        FC.savedCF = nil
    end
...
-- Chat Logger (FIXED - Target system, tampil isi, batch update)
...
-- ⭐ Batch update UI tiap 0.5 detik (biar gak berat)
task.spawn(function()
    while getgenv()._XKID_RUNNING do
        ...
        if chatLogPanel and State.Utility.chatLog then
            ...
            if #logText == 0 then logText = "Belum ada chat..." end
...
task.spawn(function()
    ...
    while getgenv()._XKID_RUNNING do
        ...
        pcall(function() if securityLabel then local afk = State.Security.afkActive and "🟢 ONLINE" or "🔴 OFFLINE"; local sl = State.Security.shiftLock and "🟢 LOCKED" or "🔴 UNLOCKED"; local vd = State.Fly.voidConn and "🟢 ACTIVE" or "🔴 INACTIVE"; local lag = State.Security.antiLag and "🟢 ACTIVE" or "🔴 INACTIVE"; securityLabel:SetDesc(string.format("[ ⏰ ] <font face='RobotoMono'>AFK Protocol :</font> %s\n[ 🔒 ] <font face='RobotoMono'>Shift Lock   :</font> %s\n[ 🕳️ ] <font face='RobotoMono'>Void Shield  :</font> %s\n[ ⚡ ] <font face='RobotoMono'>Frame Boost  :</font> %s", afk, sl, vd, lag)) end end)
    end
end)
...
secFC:Toggle({ Title = "Enable", Value = false, Callback = function(v)
    FC.active = v
    if v then
...
local function resetFilterOnly() for _, v in pairs(Lighting:GetChildren()) do if v.Name == "_XKID_CC" or v.Name == "_XKID_BLOOM" then v:Destroy() end end; Lighting.GlobalShadows = true; Lighting.Brightness = 1; Lighting.ClockTime = 14; Lighting.Ambient = Color3.new(0,0,0); Lighting.OutdoorAmbient = Color3.new(0.5,0.5,0.5); Lighting.ExposureCompensation = 0; Lighting.FogEnd = 500; State.Filter.current = "Default"; State.Filter.fullBright = false end
...
chatTargetsLabel = secChat:Paragraph({ Title = "Target", Desc = "None" })
addTargetDrop = secChat:Dropdown({ Title = "Select Target", Values = getDisplayNames(), Callback = function(v) local p = findPlayerByDisplay(v); if p then State.Utility.chatTarget = p; pcall(function() chatTargetsLabel:SetDesc("Tracking: "..p.DisplayName) end); notify("Chat", "Tracking: "..p.DisplayName.." ✅", 2) end end})
secChat:Button({ Title = "Clear Target", Callback = function() State.Utility.chatTarget = nil; pcall(function() chatTargetsLabel:SetDesc("None") end); pcall(function() addTargetDrop:Refresh(getDisplayNames(), true) end); notify("Chat", "Target cleared ❌", 2) end })
secChat:Button({ Title = "🔄 Refresh List", Callback = function() pcall(function() addTargetDrop:Refresh(getDisplayNames(), true) end); notify("Utility", "Refreshed ✅", 2) end })
...
secSrv:Button({ Title = "Server Hop", Callback = function()
    ...
    task.spawn(function() pcall(function()
        ...
        local body = HttpService:JSONDecode(r.Body); if not body or not body.data or #body.data==0 then notify("Hop","No servers",2); return end
