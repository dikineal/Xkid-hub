...
local function findPlayerByDisplay(str)
    for _, p in pairs(Players:GetPlayers()) do 
        if p.DisplayName == str then return p end 
    end
    return nil
end
...
local function toggleShiftLock(v)
    ...
    if v then
        ...
        if State.Security.shiftLockGyro then
            State.Security.shiftLockGyro:Destroy(); State.Security.shiftLockGyro = nil
        end
...
-- ══════════════════════════════════════════════════════════════
--  💎 AVATAR REFRESH ENGINE (FAST RESPAWN & BLINK REFRESH)
-- ══════════════════════════════════════════════════════════════
local function fastRespawn()
    ...
    notify("🔄 Fast Respawn", "Membunuh & Respawning...", 1.5)
    ...
    char:BreakJoints() -- Force Kill Server-side

    task.delay(4, function() State.Avatar.isRefreshing = false end)
end
...
local function refreshCharacter()
    ...
    State.Avatar.isRefreshing = true
    notify("🔄 Refresh", "Me-reload karakter (Blink)...", 1.5)

    local savedCF = hrp.CFrame
    local camCF = Cam.CFrame
    
    -- Kunci Kamera agar tidak melompat ke spawn
    ...
    -- Trigger server untuk me-load ulang karakter (Tanpa Mati)
    pcall(function() LP:LoadCharacter() end)
    
    -- EMERGENCY FAILSAFE: Jika 3 detik tidak spawn, paksa lepas kamera agar tidak bug
    task.delay(3, function()
        if State.Avatar.isRefreshing or Cam.CameraType == Enum.CameraType.Scriptable then
            ...
            notify("⚠ Info", "Kamera di-reset oleh Failsafe", 2)
        end
    end)
end
...
task.spawn(function()
    while getgenv()._XKID_RUNNING do
        ...
        task.wait(1.5) -- Sedikit delay tambahan untuk menghemat performa CPU (Optimized)
    end
end)
    ...
    RS:BindToRenderStep("XKIDSpec", Enum.RenderPriority.Camera.Value+1, function()
        ...
        pcall(function()
            if not Spec.target or not Spec.target.Parent or not Spec.target.Character or not Spec.target.Character:FindFirstChild("HumanoidRootPart") then
                notify("Spectate", "❌ Target keluar/mati!", 2); Spec.active = false; stopSpecLoop(); stopSpecCapture()
...
secChangelog:Paragraph({ Title = "Latest Updates", Desc  = "• FIX: Camera Bug di Refresh Character\n• FIX: List Spectate (Hanya DisplayName)\n• GC Memory Background (Optimal)\n• Shift Lock & Failsafe Timer" })
    ...
    while getgenv()._XKID_RUNNING do
        ...
        if securityLabel then
            securityLabel:SetDesc(string.format("🛡️ Script: Active\n👥 Players: %d\n⏰ Anti-AFK: %s\n🔒 Shift Lock: %s\n⚡ Anti-Lag: %s\n💾 Memory: GC Running",
                #Players:GetPlayers(), State.Security.afkConn and "✅" or "⭕", State.Security.shiftLock and "🔒" or "🔓", State.Security.antiLag and "✅" or "⭕"))
        end
    end
end)
...
secAvatar:Button({ Title = "Fast Respawn", Desc = "Bunuh diri & respawn di posisi ini", Callback = function() fastRespawn() end })
secAvatar:Button({ Title = "Refresh Character", Desc = "Reload avatar (Blink Mode) tanpa mati & anti camera bug", Callback = function() refreshCharacter() end })

local secMov = T_AV:Section({ Title = "Movement", Opened = true })
secMov:Button({ Title = "Refresh POV", Desc = "Reset camera & character physics", Callback = function()
    local h=getHum()
    if not h then notify("Refresh","❌ Character not found"); return end
...
secMov:Slider({ Title = "Jump Power", Step=1, Value={Min=50, Max=500, Default=50}, Callback = function(v) State.Move.jp = tonumber(v) or 50; if getHum() then getHum().UseJumpPower=true; getHum().JumpPower=State.Move.jp end end })
...
secAbi:Toggle({ Title = "Fly", Desc = "Fly following camera", Value=false, Callback = function(v) toggleFly(v) end })
...
secAbi:Toggle({ Title = "No Fall Damage", Value=false, Callback = function(v)
...
secAbi:Toggle({ Title = "God Mode (RISK DETECTED)", Desc = "Warning: Bisa di-kick anticheat (math.huge health)", Value=false, Callback = function(v)
    if v then
        ...
        TrackC(LP.CharacterAdded:Connect(function(char)
            task.wait(0.2); local hrp=char:WaitForChild("HumanoidRootPart",5); if hrp and godLastPos then hrp.CFrame=godLastPos end
            local h=char:WaitForChild("Humanoid",5); if h then h.MaxHealth=math.huge; h.Health=math.huge end
        end))
        notify("God Mode","🛡 Active! Awas kick di game ketat.", 3)
    else
...
secTP:Input({ Title = "Search Player (DisplayName)", Placeholder = "type display name...", Callback = function(v) tpTarget = v end })
secTP:Button({ Title = "Teleport", Callback = function()
    pcall(function()
        ...
        if not targetPlayer or not targetPlayer.Parent or not targetPlayer.Character then notify("Teleport", "❌ Player tidak valid/belum spawn!", 2); return end
...
secTP:Button({ Title = "Refresh List", Callback = function()
    local newList = getDisplayNames(); pcall(function() tpDropdown:Refresh(newList, true) end)
...
local specDropdown = secSP:Dropdown({ Title = "Target Player (DisplayName)", Values = specDropOpts, Callback = function(v)
    ...
    else notify("Spectate", "❌ Player kembar/tidak valid!", 2) end
end})
...
secWea:Button({ Title="🌅 Pagi", Callback = function() Lighting.ClockTime = 7; Lighting.Brightness = 1; Lighting.FogEnd = 1000 end })
secWea:Button({ Title="☀ Siang", Callback = function() Lighting.ClockTime = 14; Lighting.Brightness = 2; Lighting.FogEnd = 1000 end })
secWea:Button({ Title="🌃 Malam", Callback = function() Lighting.ClockTime = 0; Lighting.Brightness = 0.5; Lighting.FogEnd = 1000 end })
...
secAtmos:Toggle({ Title="Fullbright", Value=false, Callback=function(v)
    if v then Lighting.Ambient=Color3.new(1,1,1); Lighting.FogEnd=999999 else Lighting.Ambient=Color3.new(0.5,0.5,0.5); Lighting.FogEnd=1000 end
end})

local secGfx = T_WO:Section({ Title = "Graphics", Opened = false })
local function setGfx(level) pcall(function() settings().Rendering.QualityLevel = level end); notify("Gfx", "✅ Lv "..level, 2) end
...
secProt:Toggle({ Title="Anti-AFK", Value=false, Callback=function(v)
    if v then State.Security.afkConn = TrackC(LP.Idled:Connect(function() VirtualUser:CaptureController(); VirtualUser:ClickButton2(Vector2.new()); task.wait(1) end))
...
secTheme:Dropdown({ Title="Theme", Values=(function() local n={}; for k in pairs(WindUI:GetThemes()) do table.insert(n,k) end; table.sort(n); return n end)(), Value="Crimson", Callback=function(v) WindUI:SetTheme(v) end })
...
WindUI:Notify({ Title = "@WTF.XKID", Content = "Script Optimized & Bugs Fixed", Duration = 3 })
