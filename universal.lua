...
╚══════════════════════════════════════════════════════════════════╝
  ...
  • Modern Hybrid ESP (Highlight Mode + Glitch Detection)
...
-- ══════════════════════════════════════════════════════════════
--  💎 HYBRID DETECTION ESP ENGINE (ENHANCED)
-- ══════════════════════════════════════════════════════════════
                    ...
                    for _, v in pairs(p.Character:GetChildren()) do
                        ...
                        elseif v:IsA("Accessory") then
                            ...
                            if h and h:IsA("BasePart") then
                                if h.Size.Magnitude > 15 then
                                    isSus = true; reason = "Huge Hat" break
                                elseif h.Size.Magnitude > 5 or h.Transparency > 0.9 or h.Material == Enum.Material.Neon then
...
secChangelog:Paragraph({
    ...
    Desc  = "• Fixed Spectate Player\n• Normalized Freecam Speed\n• Enhanced ESP (Highlight Mode + Glitch Detection)\n• Added Chat Bypass Toggle\n• Weather Default & Graphics Complete\n• Removed Anti Screen-Block"
})

local secCreditsHome = T_HOME:Section({ Title = "💎 Credits", Opened = false })
secCreditsHome:Paragraph({
    Title = "Created by",
    Desc  = "@WTF.XKID\nPowered by WindUI\nVersion: V.2.2 - Enhanced"
})
...
task.spawn(function()
    while getgenv()._XKID_RUNNING do
        ...
        if statusLabel then
            ...
            local gameName = pcall(function() return game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name end) or "Unknown"
...
-- ══════════════════════════════════════════════════════════════
--  TAB 7: UTILITY (CHAT BYPASS)
-- ══════════════════════════════════════════════════════════════
local T_UTIL = Window:Tab({ Title = "Utility", Icon = "tool" })

local secChat = T_UTIL:Section({ Title = "Chat Features", Opened = true })
secChat:Toggle({
    Title    = "Chat Bypass / Anti-Filter",
    Desc     = "Send uncensored messages (auto-adds zero-width space)",
    Value    = false,
    Callback = function(v)
        State.Chat.bypass = v
        notify("Chat Bypass", v and "✅ Enabled" or "❌ Disabled", 2)
    end,
})

secChat:Paragraph({
    Title = "📝 Usage",
    Desc = "Type normally in chat. Messages will be automatically bypassed.\nCommands like /re still work normally."
})

-- ══════════════════════════════════════════════════════════════
--  TAB 8: SECURITY (ANTI SCREEN-BLOCK REMOVED)
-- ══════════════════════════════════════════════════════════════
...
secCredit:Paragraph({ Title = "Version", Desc  = "V.2.2 - Enhanced Edition" })
...
WindUI:Notify({ Title = "@WTF.XKID", Content = "Script Loaded — Enhanced Edition", Duration = 3 })
task.wait(1.5)
WindUI:Notify({ Title = "Hybrid ESP Enhanced", Content = "Highlight Mode + Glitch Detection Active", Duration = 5 })
WindUI:Notify({ Title = "⚡XKID HUB", Content = "Spectate Fixed • Chat Bypass Ready", Duration = 4 })
print("✅ @WTF.XKID Script Loaded | Enhanced Edition | All Features Working")