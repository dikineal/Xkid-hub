...
╠══════════════════════════════════════════════════════════════╣
║   ✦  Fly V2 — Karakter ikut arah kamera (CFrame-based)      ║
║   ✦  Lock Karakter Fix (solid, no rubber-band)              ║
║   ✦  Respawn Auto-Save Posisi                               ║
╚══════════════════════════════════════════════════════════════╝
]]
...
-- ┌─────────────────────────────────────────────────────────┐
-- │   ➤  FLY ENGINE V2 — Karakter ikut arah kamera         │
-- │      (Character CFrame-based, bukan BodyVelocity saja)  │
-- └─────────────────────────────────────────────────────────┘
...
-- Fly menggunakan accumulated yaw/pitch dari kamera LANGSUNG
-- Karakter bergerak sesuai CFrame kamera → natural & konsisten
local function startFlyCapture()
    ...
    -- Touch: kiri=gerak, kanan=scroll (kamera roblox default)
    table.insert(flyConns, UIS.InputBegan:Connect(function(inp, gp)
    ...
    table.insert(flyConns, UIS.TouchMoved:Connect(function(inp)
        if inp == flyMoveTouch and flyMoveSt then
            ...
            flyJoy = Vector2.new(
                math.abs(dx) > 20 and math.clamp((dx - math.sign(dx)*20)/80,-1,1) or 0,
...
local function toggleFly(v)
    ...
    -- BodyGyro: karakter menghadap arah gerak
    ...
    -- ── FLY LOOP: gunakan CFrame kamera LANGSUNG ────────────
    -- Karakter bergerak sesuai arah pandang kamera (seperti freecam lock)
    RS:BindToRenderStep("XKIDFly", Enum.RenderPriority.Camera.Value + 1, function(dt)
        if not State.Fly.active then return end
        local r = getRoot()
        local h = getHum()
        if not r or not h or not State.Fly.bv or not State.Fly.bg then return end

        -- Ambil CFrame kamera saat ini sebagai referensi arah
        ...
        if onMobile then
            -- Analog joystick kiri: maju/mundur/kiri/kanan ikut kamera
            local fwd   = Vector3.new(camCF.LookVector.X, 0, camCF.LookVector.Z).Unit
            local right = Vector3.new(camCF.RightVector.X, 0, camCF.RightVector.Z).Unit
            move = fwd  * (-flyJoy.Y) * spd
                 + right * flyJoy.X   * spd
        else
            -- WASD ikut arah kamera (horizontal plane agar tidak menukik)
            local fwd   = Vector3.new(camCF.LookVector.X, 0, camCF.LookVector.Z).Unit
            local right = Vector3.new(camCF.RightVector.X, 0, camCF.RightVector.Z).Unit
            if keys[Enum.KeyCode.W] then move = move + camCF.LookVector  * spd end  -- W: ikut pitch kamera penuh (bisa naik/turun)
            if keys[Enum.KeyCode.S] then move = move - camCF.LookVector  * spd end
            if keys[Enum.KeyCode.D] then move = move + right * spd end
            if keys[Enum.KeyCode.A] then move = move - right * spd end
            if keys[Enum.KeyCode.E] then move = move + Vector3.new(0,1,0) * spd end  -- E: naik murni
            if keys[Enum.KeyCode.Q] then move = move - Vector3.new(0,1,0) * spd end  -- Q: turun murni
        end

        State.Fly.bv.Velocity = move

        -- Gyro: karakter menghadap ke arah gerak (yaw only, no pitch)
        -- Ambil yaw dari kamera saja → karakter selalu hadap depan kamera
        local _, camYaw, _ = camCF:ToEulerAnglesYXZ()
        State.Fly.bg.CFrame = CFrame.new(r.Position) * CFrame.Angles(0, camYaw, 0)
    end)

    Library:Notification("Fly", "✈️ Fly ON — W/S ikut kamera | E=Naik Q=Turun", 3)
end
...
local function stopFCCapture()
    ...
    fcConns = {}
    fcRotTouch = nil; fcMoveTouch = nil
    fcMoveSt = nil; fcRotLast = nil
    fcJoy = Vector2.zero
    FC._mouseRotate = false
    FC._keys = {}
    UIS.MouseBehavior = Enum.MouseBehavior.Default
...
-- │                   ➤  UI CONSTRUCTION                    │
-- └─────────────────────────────────────────────────────────┘
local Win = Library:Window("✦ XKID HUB — V.2 ✦", "star", "FREECAM", false)
...
TPT:Button("🚀 Teleport Now", "Fast TP", function()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LP and (string.find(string.lower(p.Name), string.lower(snippet)) or
           string.find(string.lower(p.DisplayName), string.lower(snippet))) then
...
PLM:Button("🔄 Refresh POV", "Reset kamera & karakter", function()
    local h = getHum()
    if not h then Library:Notification("Refresh", "❌ Karakter tidak ditemukan!", 2); return end
...
PLH:Toggle("✈️  Fly", "nf", false, "W/S ikut kamera | E=Naik Q=Turun", function(v) toggleFly(v) end)
...
PLH:Toggle("🛡️ God Mode", "god", false, "HP Infinite + Respawn", function(v)
    if v then
        ...
        Library:Notification("God Mode", "🛡️ Aktif!", 3)
    else
...
-- ── PAGE 2: LOCK KARAKTER (Fix — tanpa lock kamera) ────────
local PLPage2 = T_PL:Page("Lock", "lock")
local PLLock  = PLPage2:Section("🔒 Lock Karakter", "Left")

-- ── Lock Rotasi: AutoRotate = false (metode official Roblox)
PLLock:Toggle("🔒 Lock Rotasi", "lockrot", false, "Karakter tidak berputar otomatis", function(v)
    local hum = getHum()
    if not hum then Library:Notification("Lock", "❌ Karakter tidak ditemukan!", 2); return end
    hum.AutoRotate = not v
    -- Juga batalkan momentum rotasi yang sedang berjalan
    local r = getRoot()
    if r and v then
        pcall(function() r.AssemblyAngularVelocity = Vector3.zero end)
    end
    Library:Notification("Lock", v and "🔒 Rotasi dikunci!" or "🔓 Rotasi bebas", 2)
end)

-- ── Lock Posisi (solid): Anchor + loop paksa CFrame tiap Heartbeat
-- Lebih solid dari Stepped karena Heartbeat berjalan setelah physics step
...
PLLock:Toggle("📍 Lock Posisi", "lockpos", false, "Karakter diam total", function(v)
    if v then
        ...
        lockedCF     = hrp.CFrame
        hrp.Anchored = true
        -- Paksa velocity nol saat lock aktif
        pcall(function()
        ...
        lockPosConn = RS.Heartbeat:Connect(function()
            ...
            -- Cek apakah posisi bergeser (misal di-push orang lain)
            if (r.Position - lockedCF.Position).Magnitude > 0.05 then
                r.CFrame = lockedCF
                pcall(function()
...
-- ── Lock Rotasi Arah (paksa karakter selalu hadap arah tertentu)
local lockDirConn = nil
local lockedYaw   = 0
PLLock:Toggle("🧭 Lock Arah Hadap", "lockdir", false, "Karakter selalu hadap arah ini", function(v)
    if v then
        local hrp = getRoot()
        if not hrp then Library:Notification("Lock", "❌ Karakter tidak ditemukan!", 2); return end
        -- Simpan yaw sekarang
        ...
        lockDirConn = RS.Heartbeat:Connect(function()
            local r = getRoot()
            if not r then return end
            -- Paksa rotasi yaw tetap
            local pos = r.Position
            r.CFrame = CFrame.new(pos) * CFrame.Angles(0, lockedYaw, 0)
            pcall(function() r.AssemblyAngularVelocity = Vector3.zero end)
        end)
        Library:Notification("Lock", "🧭 Arah hadap dikunci!", 2)
    else
        if lockDirConn then lockDirConn:Disconnect(); lockDirConn = nil end
        local hum = getHum()
        if hum then hum.AutoRotate = true end
        Library:Notification("Lock", "🔓 Arah bebas", 2)
    end
end)
...
local function stopSpecCapture()
    ...
    specConns = {}; specTouchMain = nil; specTouchPinch = {}
    specPinchDist = nil; specPanDelta = Vector2.zero
end
...
SPF:Button("🔭 Zoom In  [ FOV - ]",  "Makin sempit",      function() local f = math.clamp(Cam.FieldOfView-10,10,120); Cam.FieldOfView=f; Library:Notification("FOV","FOV: "..f,1) end)
SPF:Button("🔭 Zoom In  [ FOV -- ]", "Makin sempit cepat",function() local f = math.clamp(Cam.FieldOfView-30,10,120); Cam.FieldOfView=f; Library:Notification("FOV","FOV: "..f,1) end)
SPF:Button("👁️ Reset Normal (70)",  "",                  function() Cam.FieldOfView=70; Library:Notification("FOV","FOV: 70 (Normal)",1) end)
SPF:Button("🌐 Zoom Out [ FOV + ]",  "Makin lebar",       function() local f = math.clamp(Cam.FieldOfView+10,10,120); Cam.FieldOfView=f; Library:Notification("FOV","FOV: "..f,1) end)
SPF:Button("🌐 Zoom Out [ FOV ++ ]", "Makin lebar cepat", function() local f = math.clamp(Cam.FieldOfView+30,10,120); Cam.FieldOfView=f; Library:Notification("FOV","FOV: "..f,1) end)
...
WOA:Slider("☀️ Brightness",   "wbright", 0, 5,     1,      function(v) Lighting.Brightness = v end)
WOA:Slider("🌫️ Fog Jarak",    "wfog",    0, 5000,  100000, function(v) Lighting.FogEnd     = v end)
WOA:Slider("💨 Density",       "wdens",   0, 1,     0,      function(v) getAtmos().Density  = v end)
WOA:Slider("🌅 Offset (Haze)", "woffset", 0, 1,     0,      function(v) getAtmos().Offset   = v end)
WOA:Slider("✨ Glare",         "wglare",  0, 1,     0,      function(v) getAtmos().Glare    = v end)
WOA:Slider("🌟 Halo",          "whalo",   0, 1,     0,      function(v) getAtmos().Halo     = v end)
...
WOG:Button("🥔 Potato (Level 1)",          "Paling hemat",        function() setGfx(Enum.QualityLevel.Level01); Library:Notification("Graphics","🥔 Potato",2) end)
WOG:Button("📉 Low (Level 3)",             "Ringan",              function() setGfx(Enum.QualityLevel.Level03); Library:Notification("Graphics","📉 Low",2) end)
WOG:Button("📊 Medium (Level 5)",          "Seimbang",            function() setGfx(Enum.QualityLevel.Level05); Library:Notification("Graphics","📊 Medium",2) end)
WOG:Button("📈 High (Level 8)",            "Bagus",               function() setGfx(Enum.QualityLevel.Level08); Library:Notification("Graphics","📈 High",2) end)
WOG:Button("💎 Ultra (Level 10)",          "Maksimal",            function() setGfx(Enum.QualityLevel.Level10); Library:Notification("Graphics","💎 Ultra",2) end)
...
WOGF:Button("▲ Naik Level",         "Grafik lebih tinggi", function() local c = settings().Rendering.QualityLevel.Value; local n = math.clamp(c+1,1,10); setGfx(n); Library:Notification("Graphics","Level: "..n,1) end)
WOGF:Button("▼ Turun Level",        "Grafik lebih rendah", function() local c = settings().Rendering.QualityLevel.Value; local n = math.clamp(c-1,1,10); setGfx(n); Library:Notification("Graphics","Level: "..n,1) end)
WOGF:Button("🔄 Cek Level Sekarang","",                    function() Library:Notification("Graphics","Level: "..settings().Rendering.QualityLevel.Value,3) end)
...
-- ── Respawn: simpan posisi otomatis setiap detik (tanpa toggle) ──
local respawnLastPos = nil

-- Auto-save posisi berjalan selamanya selama script aktif
task.spawn(function()
    while true do
        local r = getRoot()
...
SCR:Button("📌 Simpan Posisi Sekarang", "Simpan manual (override auto)", function()
    local r = getRoot()
    if r then
        respawnLastPos = r.CFrame
        Library:Notification("Respawn", "📌 Posisi tersimpan manual!", 2)
    else
...
SCR:Button("💀 Fast Respawn", "Mati & balik ke posisi terakhir", function()
    if not respawnLastPos then
        Library:Notification("Respawn", "❌ Posisi belum tersimpan!", 2)
        return
    end
    local savedCF = respawnLastPos
    local hum = getHum()
    if not hum then Library:Notification("Respawn", "❌ Karakter tidak ditemukan!", 2); return end
    hum.Health = 0
    task.spawn(function()
...
Library:Notification("✦ XKID HUB", "V.2 — Fly Fix + Lock Fix + Auto Respawn! 🚀", 5)
