...
local Spec = {
    ...
    dist     = 8,     -- jarak kamera third-person
    conn     = nil,
    origFov  = 70,
}
...
local specDrop = SPS:Dropdown("Pilih Target", "spDrop", getPNames(), function(v)
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p.Name == v then Spec.target = p; break end
    end
end)
SPS:Button("🔄 Refresh", "", function() specDrop:Refresh(getPNames()) end)

SPS:Toggle("👁️ Spectate ON/OFF", "spec", false, "Nonton sudut pandang target", function(v)
    Spec.active = v
    if v then
        if not Spec.target then
            Library:Notification("Spectate", "Pilih target dulu!", 3)
            -- matiin toggle kembali
            Spec.active = false
            return
        end
        ...
        Spec.conn = RS.RenderStepped:Connect(function()
            if not Spec.active then return end
            local char = Spec.target and Spec.target.Character
            local hrp  = char and char:FindFirstChild("HumanoidRootPart")
            if not hrp then return end

            if Spec.mode == "first" then
                -- First person: kamera tepat di kepala target
                local head = char:FindFirstChild("Head")
                local cf   = head and head.CFrame or hrp.CFrame
                Cam.CFrame = cf * CFrame.new(0, 0, -0.5)
            else
                -- Third person: di belakang target, jarak bisa diatur
                local back = hrp.CFrame * CFrame.new(0, 2, Spec.dist)
                Cam.CFrame = CFrame.new(back.Position, hrp.Position)
            end
        end)
        Library:Notification("Spectate", "Nonton: " .. Spec.target.Name, 3)
    else
        if Spec.conn then Spec.conn:Disconnect(); Spec.conn = nil end
        Cam.CameraType = Enum.CameraType.Custom
...
SPS:Toggle("🎥 First Person", "specfp", false, "ON = FP | OFF = 3rd", function(v)
    Spec.mode = v and "first" or "third"
end)

-- Slider jarak kamera (third person)
SPS:Slider("Jarak Kamera", "specdist", 3, 30, 8, function(v)
    Spec.dist = v
end)

-- FOV Zoom — bisa zoom in/out kapan saja (spectate atau tidak)
SPF:Slider("🔍 FOV (Zoom)", "fovzoom", 10, 120, 70, function(v)
    Cam.FieldOfView = v
end)
SPF:Button("🔭 Zoom In (30)", "", function()
    Cam.FieldOfView = 30
end)
SPF:Button("👁️ Normal (70)", "", function()
    Cam.FieldOfView = 70
end)
SPF:Button("🌐 Zoom Out (100)", "", function()
    Cam.FieldOfView = 100
end)
